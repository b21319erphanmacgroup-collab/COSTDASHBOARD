...
$GB_user_id 	= $_SESSION["CK_memberID"];		//사원번호
$GB_user_id 	= $GB_user_id==""?$_SESSION['SS_memberID']:$GB_user_id;
...
// 					$this->smarty->assign('projectHistory',$array_getLastAddWork);
					...
					if ($GB_user_id == "B21319") {
						$this->smarty->display("intranet/common/searchPop_projectCode2.tpl");
					} else {
					...
					if(change_XXIS02($d_EntryPCode,'HANM')){ // XX관련 코드 => 리턴값이 true
						$d_EntryPCode_edit = "XX"."-".$arrayStr2."-".$arrayStr3;
					}//if End

					echo '='.$d_EntryPCode_edit.'=';
...
// 							//============================================================================
						...
						if($pjt_product_cd == 'BM' or $pjt_product_cd == 'BG'){
							$pjt_year = '0000';
						}
						//============================================================================
						$sql_add_01 = "";
						...
						if($pjt_product_cd && $pjt_product_cd!=""){
							$sql_add_02 .= "	AND															";
							$sql_add_02 .= "	substring(P.NewProjectCode, 1, 2) ='$pjt_product_cd'		";
						}
						//사업년도
						if($pjt_year && $pjt_year!=""){
							$pjt_year = substr($pjt_year,2,2);
						...
						$sql .= "	 	 and P.NewprojectCode is not null		";

						$sql .= "	 	 and P.visible_YN = 'Y'					";
						$sql .= "
								 ORDER BY
								 substring(P.projectViewCode, 4, 2) desc
								 ,substring(P.projectViewCode, 7, 2)
								 ,CONVERT(substring(P.projectViewCode, 10, 2), UNSIGNED)
								";
					
						// if($GB_user_id == 'B23030'){echo $sql;}
 					//echo $sql;
// 					exit();
						...
						if($result_num != 0)
						{
							...
							{
								/*---------------------------------*/
								$re_row[projectCode] = change_code($re_row[projectCode]);

								//$strCheck = $re_row[P_CodeCheck01];
								if ($GB_user_id == "B21319") {
									$re_row[ProjectNicknameShort] = getStrShort($re_row[ProjectNickname], 35, '');
								} else {
									$re_row[ProjectNicknameShort] = getStrShort($re_row[ProjectNickname], 18, '');
								}
