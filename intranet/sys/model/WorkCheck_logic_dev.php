	...
	function ViewPage(){
		...
		switch($MainAction){
			...
			default:
				...
				$sql="
							SELECT 
								DATE_FORMAT(EntryTime,'%Y-%m-%d') AS latestConfirmDt
							FROM 
								dallyproject_tbl 
							WHERE 
								ReportingBoss='".$_SESSION["memberID"]."' 
							AND IFNULL(WorkConfirmYN,'N') = 'Y'
				...
				if(mysql_num_rows($re) > 0 ){
					$latestComfirmDate = mysql_result($re,0,'latestConfirmDt');
				}
				...
				$this->assign('sel_date',date('Y-m-d', strtotime("-9 day")));
				$this->assign('latestConfirmDt',$latestComfirmDate);
				
				$this->display("intranet/common_contents/work_check_dev/work_check_board_mvc.tpl");
			break;
		}
	}
	...
	function WorkCheck_DayList_01(){
		...
		switch($SubAction){
			default:
				...
				$this->display("intranet/common_contents/work_check_dev/work_check_daylist_mvc.tpl");
			break;
		}
	}
	...
	function WorkCheck_MemberWorkList_01(){
		...
		switch($SubAction){
			default:
				...
				$this->display("intranet/common_contents/work_check_dev/work_check_memworklist_mvc.tpl");
			break;
		}
	}
	...
	function WorkCheck_MemberWorkConfirm_01(){
		...
		$sql="Update
				...
				SET
					WorkConfirmYN='Y'
				WHERE
					DATE_FORMAT(EntryTime,'%Y-%m-%d') BETWEEN '".$latestConfirmDt."' AND  '".$selectDate."'
