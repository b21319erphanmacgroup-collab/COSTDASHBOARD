...
class StudyLogic {
	...
	function StudyLogic($smarty) {
		session_start ();
		$Controller = "Study_Controller2.php";
	...
	function MyClass_Mobile() {
		...
		switch ($MainAction) {
			...
			case "HanmacCulture" :
				...
				break;
			default :
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_HanmacPick_Main.tpl" );
				break;
		}
	}
		...
		switch ($SubAction) {
			case "Main" :
				...
				if (array_key_exists ( $type, $impressionTypes )) {
					...
					$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_HanmacPick_Impression_Main.tpl" );
					...
					$sql .= "             '../../image/phonebook/thumb_noimage.png'  ";
					$sql .= "     End preview_img,  ";
					$sql .= "     Date_Format(d.reg_dt, '%Y-%m-%d') disp_reg_dt ";
					$sql .= " From ";
					$sql .= "     ( ";
					$sql .= "         Select ";
					$sql .= "             b.*, ";
					...
					$sql .= "                         Select  ";
					$sql .= "                             Case  ";
					$sql .= "                                 When TimeStampDiff(Second, x.reg_dt, Sysdate()) > 86400 Then 'F'   ";
					$sql .= "                                 Else   ";
					$sql .= "                                     'T'  ";
					$sql .= "                             End NEWCONTENTS_TF  ";
					$sql .= "                         From  ";
					$sql .= "                             class_recommend_new_tbl x  ";
					$sql .= "                         Where x.category = 'A'  ";
					$sql .= "                         And x.hide_yn = 'N'  ";
					$sql .= "                         And x.hanmacPick_cd = a.hanmacPick_cd  ";
					$sql .= "                         And x.seq = a.seq ";
					$sql .= "                     ) NEWCONTENTS_TF ";
					$sql .= "                 From  ";
					$sql .= "                     (  ";
					...
					$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_HanmacPick_Comment_Main.tpl" );
				}
			...
			case "Detail" :
				...
				$sql .= "         Where x.seq = a.seq ";
				$sql .= "     ) retrieve_cnt, ";
				$sql .= "     Date_Format(a.reg_dt, '%Y-%m-%d') disp_reg_dt ";
				$sql .= " From  ";
				...
				while ( $row = mysql_fetch_array ( $resource, MYSQL_ASSOC ) ) {
					if ($row [contents_cd] == "B") {
						...
						$row ["pdf_path"] = "http://$_SERVER[HTTP_HOST]/intranet_file/classRecommend/file/$row[seq]/pdf/$row[pdf_file_name]";
					} else {
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_HanmacPick_Comment_Detail.tpl" );
				break;
			case "saveComment" :
				if (empty ( $seq )) {
			...
			case "ImpressionView" :
				...
				if ($type == "C") {
					...
					$sql .= "             'F'   ";
					$sql .= "     End EDIT_TF,      ";
					$sql .= "     b.contents user_impression,      ";
					$sql .= "     c.contents_cd,      ";
					$sql .= "     c.file_path,      ";
					$sql .= "     c.file_name      ";
					$sql .= " From ";
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_HanmacPick_Impression_Detail.tpl" );
				break;
			case "ImpressionWrite" :
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_HanmacPick_Impression_Write.tpl" );
				break;
			case "ImpressionSave" :
				$response = array (
						rstCd => 200,
			...
			default :
				...
				$sql .= "             ) comment_cnt,  ";
				$sql .= "             Case  ";
				$sql .= "                 When t.contentsType = 'history' Then t.reg_dt  ";
				$sql .= "                 Else  ";
				$sql .= "                     u.reg_dt  ";
				$sql .= "             End reg_dt,  ";
				$sql .= "             Case   ";
				$sql .= "                 When t.contentsType = 'history' Then 1 ";
				$sql .= "                 Else   ";
				$sql .= "                     2 ";
				$sql .= "             End contentsSortNo,   ";
				$sql .= "             Case  ";
				...
				$sql .= "     a.*, ";
				$sql .= "     ( ";
				$sql .= "         Select x.Name From systemconfig_tbl x Where x.SysKey = 'classRecommend_categoryCulture' And x.Code = a.category_culture_cd ";
				$sql .= "     ) leader_name ";
				$sql .= " From  ";
				$sql .= "     class_recommend_new_tbl a ";
				$sql .= " Where category = 'B' ";
				$sql .= " And slide_show_yn = 'Y' ";
				$resource = mysql_query ( $sql, $db );
				...
				$hanmacCulture = array ();
				while ( $row = mysql_fetch_array ( $resource, MYSQL_ASSOC ) ) {
					array_push ( $hanmacCulture, $row );
				}

				$sql = " Select   ";
				$sql .= "     (       ";
				$sql .= "         Select   ";
				$sql .= "             CoalEsce(Max(y.NEWCONTENTS_TF), 'F') NEWCONTENTS_TF   ";
				$sql .= "         From   ";
				$sql .= "             (   ";
				$sql .= "                 Select   ";
				$sql .= "                     Case   ";
				...
				$sql .= "                 Where x.category = 'A'   ";
				$sql .= "                 And x.hide_yn = 'N'   ";
				$sql .= "                 And x.hanmacPick_cd = 'B'   ";
				$sql .= "             ) y   ";
				$sql .= "         Where y.NEWCONTENTS_TF = 'T'   ";
				$sql .= "     ) HANMAC_KNOWLEDGE_NEWCONTENTS_TF,   ";
				$sql .= "     (   ";
				$sql .= "         Select   ";
				$sql .= "             CoalEsce(Max(y.NEWCONTENTS_TF), 'F') NEWCONTENTS_TF   ";
				$sql .= "         From   ";
				...
				$sql .= "                 Where x.category = 'A'   ";
				$sql .= "                 And x.hide_yn = 'N'   ";
				$sql .= "                 And x.hanmacPick_cd = 'F'   ";
				$sql .= "             ) y   ";
				$sql .= "         Where y.NEWCONTENTS_TF = 'T'   ";
				$sql .= "     ) LAW_LECTURE_NEWCONTENTS_TF,  ";
				$sql .= "     (   ";
				$sql .= "         Select   ";
				$sql .= "             CoalEsce(Max(y.NEWCONTENTS_TF), 'F') NEWCONTENTS_TF   ";
				$sql .= "         From   ";
				...
				$sql .= "                 Where x.category = 'A'   ";
				$sql .= "                 And x.hide_yn = 'N'   ";
				$sql .= "                 And x.hanmacPick_cd = 'E'   ";
				$sql .= "             ) y   ";
				$sql .= "         Where y.NEWCONTENTS_TF = 'T'   ";
				$sql .= "     ) MONTH_RECOMENT_NEWCONTENTS_TF ";
				$sql .= " From   ";
				$sql .= "     DUAL   ";
				$resource = mysql_query ( $sql, $db );

				if (! $resource) {
					echo "mysql_query(newContents resource) error";
					return;
				}

				$newContentsData = array ();
				while ( $row = mysql_fetch_array ( $resource, MYSQL_ASSOC ) ) {
					array_push ( $newContentsData, $row );
				}

				$this->smarty->assign ( "hanmacCulture", $hanmacCulture );
				$this->smarty->assign ( "recentContents", $recentContents );
				$this->smarty->assign ( "userInfo", $userInfo );
				$this->smarty->assign ( "newContentsData", $newContentsData );
	...
	function Culture() {
		...
		switch ($SubAction) {
			case "Main" :
				...
				$sql .= "                         Where x.category = 'A'   ";
				$sql .= "                         And x.hide_yn = 'N'   ";
				$sql .= "                         And x.culture_cd = a.culture_cd   ";
				$sql .= "                         And x.seq = a.seq  ";
				$sql .= "                     ) NEWCONTENTS_TF  ";
				$sql .= "                 From  ";
				$sql .= "                     (  ";
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Culture_Comment_Main.tpl" );
				break;
			case "Detail" :
				if (empty ( $type ) || empty ( $seq )) {
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Culture_Comment_Detail.tpl" );
				break;
			case "saveComment" :
				if (empty ( $seq )) {
					echo json_encode ( $this->getResponseError ( 500, "필수값 에러 ==> seq" ) );
			...
			default :
				...
				$sql .= "                 Where x.category = 'A'  ";
				$sql .= "                 And x.hide_yn = 'N'  ";
				$sql .= "                 And x.culture_cd = 'A'  ";
				$sql .= "             ) y  ";
				$sql .= "         Where y.NEWCONTENTS_TF = 'T'  ";
				$sql .= "     ) MANAGEMENT_NEWCONTENTS_TF,  ";
				$sql .= "     (      ";
				$sql .= "         Select  ";
				$sql .= "             CoalEsce(Max(y.NEWCONTENTS_TF), 'F') NEWCONTENTS_TF  ";
				$sql .= "         From  ";
				...
				$sql .= "                 Where x.category = 'A'  ";
				$sql .= "                 And x.hide_yn = 'N'  ";
				$sql .= "                 And x.culture_cd = 'B'  ";
				$sql .= "             ) y  ";
				$sql .= "         Where y.NEWCONTENTS_TF = 'T'  ";
				$sql .= "     ) HUMANITIES_NEWCONTENTS_TF,  ";
				$sql .= "     (      ";
				$sql .= "         Select  ";
				$sql .= "             CoalEsce(Max(y.NEWCONTENTS_TF), 'F') NEWCONTENTS_TF  ";
				$sql .= "         From  ";
				...
				$sql .= "                 Where x.category = 'A'  ";
				$sql .= "                 And x.hide_yn = 'N'  ";
				$sql .= "                 And x.culture_cd = 'C'  ";
				$sql .= "             ) y  ";
				$sql .= "         Where y.NEWCONTENTS_TF = 'T'  ";
				$sql .= "     ) IT_NEWCONTENTS_TF  ";
				$sql .= " From  ";
				$sql .= "     DUAL  ";
				$resource = mysql_query ( $sql, $db );
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Culture_Main.tpl" );
				break;
		}
	}
	...
	function Duty() {
		...
		switch ($SubAction) {
			case "Main" :
				...
				$sql .= "                         Where x.category = 'A'   ";
				$sql .= "                         And x.hide_yn = 'N'   ";
				$sql .= "                         And x.leader_cd = a.leader_cd   ";
				$sql .= "                         And x.seq = a.seq  ";
				$sql .= "                     ) NEWCONTENTS_TF  ";
				$sql .= "                 From  ";
				$sql .= "                     (  ";
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Duty_Comment_Main.tpl" );
				break;
			case "Detail" :
				if (empty ( $type ) || empty ( $seq )) {
					echo "Parameter Error";
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Duty_Comment_Detail.tpl" );
				break;
			case "saveComment" :
				if (empty ( $seq )) {
					echo json_encode ( $this->getResponseError ( 500, "필수값 에러 ==> seq" ) );
			...
			default :
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Duty_Main.tpl" );
				break;
		}
	}
	...
	function Leader() {
		...
		switch ($SubAction) {
			case "Main" :
				...
				$sql .= "                     a.*,  ";
				$sql .= "                     Case ";
				$sql .= "                         When DateDiff(curdate(), reg_dt) >= 1 Then 'F' ";
				$sql .= "                         Else ";
				$sql .= "                             'T' ";
				$sql .= "                     End new_contents_tf, ";
				$sql .= "                     (  ";
				$sql .= "                         Select  ";
				$sql .= "                             Count(*)  ";
				$sql .= "                         From  ";
				...
				if (! $resource) {
					echo "mysql_query(Leader resource) error";
					return;
				}
				...
				while ( $row = mysql_fetch_array ( $resource, MYSQL_ASSOC ) ) {
					...
					$row ["videoCnt"] = $this->getIframeCount ( $row ["contents"] );
					$row ["sortNo"] = $sortNoMax;
					$row ["title"] = str_replace ( "[김방희 별책부록]", "", str_replace ( "[김방희 성공예감]", "", $row ["title"] ) );
					array_push ( $fullData, $row );
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Leader_Comment_Main.tpl" );
				break;
			case "Detail" :
				if (empty ( $type ) || empty ( $seq )) {
					echo "Parameter Error";
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Leader_Comment_Detail.tpl" );
				break;
			case "saveComment" :
				if (empty ( $seq )) {
					echo json_encode ( $this->getResponseError ( 500, "필수값 에러 ==> seq" ) );
			...
			default :
				...
				$sql .= " From ";
				$sql .= "     class_recommend_new_tbl a ";
				$sql .= " Where a.leader_cd In ('A', 'B', 'C') ";
				$sql .= " And a.hanmacPick_cd Not In ('C', 'D')  ";
				$sql .= " And a.hide_yn = 'N' ";
				$sql .= " And a.category = 'A' ";
				$sql .= " Order By ";
				...
				$sql .= "                 Where x.category = 'A'  ";
				$sql .= "                 And x.hide_yn = 'N'  ";
				$sql .= "                 And x.leader_cd = 'A'  ";
				$sql .= "             ) y  ";
				$sql .= "         Where y.NEWCONTENTS_TF = 'T'  ";
				$sql .= "     ) SUCCESS_NEWCONTENTS_TF,  ";
				$sql .= "     (      ";
				$sql .= "         Select  ";
				$sql .= "             CoalEsce(Max(y.NEWCONTENTS_TF), 'F') NEWCONTENTS_TF  ";
				$sql .= "         From  ";
				...
				$sql .= "                 Where x.category = 'A'  ";
				$sql .= "                 And x.hide_yn = 'N'  ";
				$sql .= "                 And x.leader_cd = 'B'  ";
				$sql .= "             ) y  ";
				$sql .= "         Where y.NEWCONTENTS_TF = 'T'  ";
				$sql .= "     ) BONUS_BOOK_NEWCONTENTS_TF,  ";
				$sql .= "     (      ";
				$sql .= "         Select  ";
				$sql .= "             CoalEsce(Max(y.NEWCONTENTS_TF), 'F') NEWCONTENTS_TF  ";
				$sql .= "         From  ";
				...
				$sql .= "                 Where x.category = 'A'  ";
				$sql .= "                 And x.hide_yn = 'N'  ";
				$sql .= "                 And x.leader_cd = 'C'  ";
				$sql .= "             ) y  ";
				$sql .= "         Where y.NEWCONTENTS_TF = 'T'  ";
				$sql .= "     ) LEADERSHIP_NEWCONTENTS_TF  ";
				$sql .= " From  ";
				$sql .= "     DUAL  ";
				$resource = mysql_query ( $sql, $db );
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Leader_Main.tpl" );
				break;
		}
	}
	...
	function HanmacCulture() {
		...
		$sql .= " Where x.SysKey = 'classRecommend_categoryCulture' ";
		$sql .= " Order By ";
		$sql .= "     orderno2 ";
		$resource = mysql_query ( $sql, $db );
		...
		while ( $row = mysql_fetch_array ( $resource, MYSQL_ASSOC ) ) {
			...
			if ($type == $row [Code]) {
				$this->smarty->assign ( "typeName", $row [Name] );
			}
		}
		...
		switch ($SubAction) {
			case "Main" :
				...
				$sql .= "         From   ";
				$sql .= "             class_recommend_new_tbl   ";
				$sql .= "         Where category_culture_cd = '$type' ";
				$sql .= "         And category = 'B' ";
				$sql .= "     ) totalCnt, ";
				$sql .= "     Case ";
				$sql .= "         When NullIf(d.thumbnail_img, '') Is Not Null Then d.thumbnail_img ";
				...
				$sql .= "                 Select  ";
				$sql .= "                     a.seq,  ";
				$sql .= "                     (Select x.Name From systemconfig_tbl x Where x.SysKey = 'classRecommend_categoryCulture' And x.Code = a.category_culture_cd) culture_category_name,  ";
				$sql .= "                     Date_Format(a.end_dt, '%Y-%m-%d') end_dt,  ";
				$sql .= "                     a.title,  ";
				$sql .= "                     a.category,  ";
				$sql .= "                     a.category_culture_cd,  ";
				$sql .= "                     a.short_desc,  ";
				$sql .= "                     a.thumbnail_img,  ";
				$sql .= "                     a.img_path,  ";
				$sql .= "                     (  ";
				...
				$sql .= "                         From   ";
				$sql .= "                             class_recommend_new_tbl   ";
				$sql .= "                         Where (('ALL' = '$type' And category_culture_cd In ('A', 'B', 'C')) Or category_culture_cd = '$type')  ";
				$sql .= "                         And category = 'B'  ";
				$sql .= "                     ) a   ";
				$sql .= "                 Order By  ";
				$sql .= "                     a.orderno Desc ";
				...
				if (! $resource) {
					echo "mysql_query(HanmacCulture resource) error";
					return;
				}
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_HanmacCulture_Main.tpl" );
				break;
			case "Detail" :
				if (empty ( $type ) || empty ( $seq )) {
					echo "Parameter Error";
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_HanmacCulture_Comment_Detail.tpl" );
				break;
			case "saveComment" :
				if (empty ( $seq )) {
					echo json_encode ( $this->getResponseError ( 500, "필수값 에러 ==> seq" ) );
			...
			case "deleteComment" :
				...
				break;
			default :
				echo $this->getHtmlError ( "400", "파라매터 오류입니다.<br>필수값이 없습니다." );
				break;
		}
	}
	 ...
	 * 변경이력 :
	 * 1. 2022-05-30 / 김윤하 / 김병철 / 최초작성
	 * 2. 2022-12-19 / 도병덕 / 김병철 / 검색결과 카테고리추가
	 * ****************************************************************************
	 */
	function Search() {
		extract ( $_REQUEST );
		...
		switch ($SubAction) {
			...
			case "searchResult" :
				...
				while ( $row = mysql_fetch_array ( $resource, MYSQL_ASSOC ) ) {
					...
					if ($row [hanmacPick_cd] != "A") {
						array_push ( $category, $category_name [hanmacPick] [$row [hanmacPick_cd]] );
					}
					if ($row [duty_cd] != "E") {
						array_push ( $category, $category_name [duty] [$row [duty_cd]] );
					}
					if ($row [leader_cd] != "D") {
						array_push ( $category, $category_name [leader] [$row [leader_cd]] );
					}
					if ($row [culture_cd] != "E") {
						array_push ( $category, $category_name [culture] [$row [culture_cd]] );
					}

					$row [category_name] = Join ( ",", $category );
			...
			default :
				...
				$this->smarty->display ( "intranet/common_contents/work_myclass/study2/Study_Search.tpl" );
				break;
		}
	}
	...
	function insertRetrieveLog() {
		...
		$sql = " Select ";
		$sql .= "     CoalEsce(TimeStampDiff(Second, Max(retrieve_time), sysdate()), $lmitTime) timeStampDiff ";
		$sql .= " From ";
		$sql .= "     study_retrieve_log_tbl ";
		$sql .= " Where seq = '$seq' ";
		$sql .= " And company_code = '$userInfo[Company_Kind]' ";
		$sql .= " And userno = '$userInfo[memberID]' ";
		$resource = mysql_query ( $sql, $db );

		if (! $resource) {
			echo "mysql_query(retrieve_time resource) error";
			return false;
		}

		while ( $row = mysql_fetch_array ( $resource, MYSQL_ASSOC ) ) {
			$timeStampDiff = $row [timeStampDiff];
		}

		if ($timeStampDiff < $lmitTime) {
			return true;
		}
	...
	function writeAccessLog() {
		...
		$sFolderName = "../log/study2/" . "$sCurYear/" . "$sCurMonth/";
	...
	function getUserInfo($Company_Kind, $memberID, $type = null, $year = null, $quarter = null) {
		...
		if ($type == "MyClass") {
			...
			$sql .= "         When d.Company_Kind = 'HT' Then '현타'   ";
			...
			$sql .= "         When sys_comp_code = 70 Then 'HT' ";
			$sql .= "         Else ";
			$sql .= "             Null ";
			$sql .= "     End Company_Kind,  ";
			$sql .= "     Case ";
			...
			$sql .= "         When sys_comp_code = 70 Then '현타' ";
			$sql .= "         Else ";
			$sql .= "             Null ";
			$sql .= "     End company_name, ";
			$sql .= "     dept_name dept, ";
