	...
	class PersonLoginBoard extends Smarty {
		...
		/////////////////////////////////////////////////////////
			...
			for ($iday=1;$iday<=$ilastday;$iday++)
			{
				...
				/*
				if ($SearchID == "M21420" && $SearchDate == "2026-02-02") {
					echo $sql;
					exit;
				}
				*/
				...
				if($re_num == 0) //근무기록없으면
				{
					...
					}else
					{
						...
						}else
						{
							...
							if($re_num2>0)
							{
								...
								if($re_num2 >1){
									$ProjectCode	= mysql_result($re2,1,"ProjectCode");
									$UseState		= mysql_result($re2,1,"state");
									$note			= mysql_result($re2,1,"note");
									$note			= str_replace(" ","",$note);
									$StateName		= mysql_result($re2,1,"StateName");
								}
					...
					$re_row[UseState_Is]=$UseState;
					$re_row[ViewDate] = $SearchDate;
					/* -------------------------- */
				...
				}else //근무기록 존재시
				{
						...
						while($re_row = mysql_fetch_array($re)){
							...
							//Entry/Leave변경사항으로 통 주석 처리 후 Entry만 살리기
							/*
							...
							*/
							...
							if($subSelect!=""){
								$WorkConfirmYN=$re_row["WorkConfirmYN"];
							}
									...
									if($re_num > 0) {  //휴가파견있으면
										...
										if($RankCode<="C7" || $holy_sc == "holyday"){
											...
											*/
											...
											// 240712 유재욱대리요청(서승완 연구원 적용)
											// 경영지원부 애이연 대리 출산 관련 이슈로 
											// 9:15분까지 색표시 검은색으로 적용
											if($SearchID=="M02323"){
												$re_row[IntEntryTime] = $EntryTime;
												// $this->assign("IntEntryTime",$IntEntryTime);
											}
						...
						/* /////////////////////////////////////////////////////////////////// */
							...
							if (isXXCode($CompanyKind, $arrayStr2)) {
								$d_EntryPCode_edit = getXXCode($CompanyKind)."-".$arrayStr2."-".$arrayStr3;
							}
						...
						/* /////////////////////////////////////////////////////////////////// */
							...
							$re_row["WorkConfirmYN"]="";
							
							if($subSelect!=""){
								$re_row["WorkConfirmYN"]=$WorkConfirmYN;
							}
						/////////////////////////////////////////////////////////
						// 일주일전~오늘날짜 수정가능 여부
						if (in_array($SearchDate,$search_array)) {
						...
						/////////////////////////////////////////////////////////
							...
							if($tr_count > 0) {
								$addWorkSql  = " Select * From dallyproject_addwork_tbl Where MemberNo = '$SearchID' And EntryTime = '$ViewDate' ";
								$resource = mysql_query($addWorkSql, $db);
								
								if (!$resource) {
									echo "mysql_query(addWork resource) error";
									return;
								}
								
								$addWork = array();
								while ($row = mysql_fetch_array($resource, MYSQL_ASSOC)) {
									$Str_arr = explode("-", $row["project_code"]);
									/*-------------------------*/
									$arrayStr1 = $Str_arr[0];
									$arrayStr2 = $Str_arr[1];
									$arrayStr3 = $Str_arr[2];
									/*-------------------------*/
									//관리,고문,교휴,영업,업무,지원 은 XX-로 시작함
									$d_EntryPCode_edit = $row["project_code"];
									/*-------------------------*/
									...
									/* /////////////////////////////////////////////////////////////////// */
									$addWork[0] = $row["project_code"];
									$addWork[1] = $row["activity_code"];
									//$addWork[2] = utf8_strcut($row["contents"], 20, '...');
									$addWork[2] = $row["contents"];
									$addWork[3] = sprintf("%02d", $row["work_hour"]) . ":" . sprintf("%02d", $row["work_min"]);
									$addWork[4] = $row["new_project_code"];
									$addWork[job] = $row["activity_code"];
									$addWork[contents] = $row["contents"];
									$addWork[ProjectNickname] = utf8_strcut($ProjectNickname, 8, '..');
									$addWork[ProjectViewCode] = $ProjectViewCode;
									$addWork[EntryTime] = $row["EntryTime"];
									$addWork[row_num] = $num;
									
									array_push($query_data2, $addWork);
								}
								
								$tr_count = mysql_num_rows($resource);
							}// if End
								...
								if($re_out_num != 0) //외출한 로그있으면
								{
									if($re_out_num != 0) //외출한 로그있으면
									{
										while($re_out_row = mysql_fetch_array($re_out))
										{
											...
											{
												$convertedProjectCode = projectToColumn($td_array2[0],'projectViewCode');
												$sql3=" select * from project_tbl where (ProjectViewCode='$convertedProjectCode' Or ProjectCode='$convertedProjectCode')";
