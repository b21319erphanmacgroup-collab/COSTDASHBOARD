			...
			*/
				...
				(
						...
						(
							...
							(
							 ...
							 select u.MemberNo as u_MemberNo,u.state as u_state,u.ProjectCode as u_ProjectCode,u.note as u_note,SUM(u.sub_code) as u_sub_code,s.Name as StateName from
							(
							...
							)s on u.state = s.Code GROUP BY u.MemberNo, u.state order by u.num
			...
			while($re_row = mysql_fetch_array($re)){
				...
				*/
				...
				if($u_state <> '') {  //휴가파견있으면
					...
					if($GroupCode=="003"){
						...
						if(strpos($MemberNoArr2, $MemberNo) !== false) {
								//2024.07.09 AM 09:20 유지원 사원 요청으로 건설사업관리사업부서 인원 출퇴근
								//실제는 정미희 대리 요청으로 출결관리를 좀 더 편하게 보고 싶어서 요청옴.
								//state 값을 먼저 GroupBy 로 묶어서 조회
								//GroupBy로 묶어서 조회해도 여러건이면 if, 1건이면 else if 
								$preSql = "select * from view_userstate_tbl where (start_time <= '$date' and end_time >= '$date') and MemberNo = '$MemberNo' and state <>15  GROUP BY state";
								$preRe = mysql_query($preSql,$db);
								$preRe_num = mysql_num_rows($preRe);
								
								if($preRe_num > 1){
									$sqlu2 = "select a.*,b.Name as StateName from
									(
									select * from view_userstate_tbl where (start_time <= '$date' and end_time >= '$date') and MemberNo = '$MemberNo' and state <>15 and state not in ('22','23')
									) a left JOIN
									(
									select * from systemconfig_tbl where SysKey = 'UserStateCode'
				...
				if($tr_count > 0) {
					$addWorkSql  = " Select * From dallyproject_addwork_tbl Where MemberNo = '$MemberNo' And EntryTime = '$date' ";
