...
*/
			...
			if($PersonAuthority->GetInfo($memberID,'대화B'))
			...
			}else if($Conversation_group=="98")
			{
				$sql_title="select * from conversation_title_tbl where conversation_year='$sel_year' and  conversation_title <> '' and  Conversation_group ='98' order by Conversation_step";
			...
			if($searchv=="")
			{
					if($GroupCode=="003")
					{
						$sql="select * from
								(
									select * from member_tbl where (WorkPosition = 1 or WorkPosition = 4) and GroupCode='$GroupCode' order by GroupCode,IF(RealRankCode > 'C7',RankCode,IF(RealRankCode='',RankCode, IFNULL(RealRankCode,RankCode))),binary(korName)
								)a1 left JOIN 	(
			...
			while($re_row = mysql_fetch_array($re))
			{
				...
				(
					...
					select korName,MemberNo from member_tbl
				)b on a.Manager_id=b.MemberNo";

				//$sql2="select * from view_conversation_tbl where conversation_year='$sel_year' and Employee_id='$Employee_id'";
				//echo $sql2."<br>";
...
*/
			...
			*/
			...
			}else if($ManagerGroupCode=="98")
			{
				$sql_title="select * from conversation_title_tbl where conversation_year='$Conversation_year'  and Conversation_group ='98' order by Conversation_step";
				//$sql_title="select * from conversation_title_tbl where conversation_year='$Conversation_year'  and (Conversation_group IS NULL or Conversation_group ='') order by Conversation_step";
			}
			else
			{
				$sql_title="select * from conversation_title_tbl where conversation_year='$Conversation_year'  and (Conversation_group IS NULL or Conversation_group ='') order by Conversation_step";
