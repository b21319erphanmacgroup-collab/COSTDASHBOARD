<?php
	/* ***********************************
	* 2025-12-04 : 근태 관련 내용 추가 - 김한결 (KeyWord-Entry/Leave변경사항) 2025.12.23 추가
	* 2025-10-24 : 업무검토자 관련 내용 추가 - 김한결 (KeyWord-업무검토)
	* 초기화면 로딩내용 관련
	* 2015-      :
...
class MainLogic extends Smarty {
	...
	function SelectWorkPlan()
	{
		...
		$sql.= "	 ,a.LeaveTime as LeaveTime_full								";	//업무종료 시간 /*2026-01-02 추가 김한결*/
		...
		$sql.= "	,a.LeaveJob                           						";	//업무내용
		$sql.= "	,a.ReportingBoss                           					";	//보고 대상자-업무검토
...
if(DevConfirm($memberID)){
	...
	//echo 'sql='.$sql;
		...
		if($result_num != 0) {
			...
			$LeaveTime_full     = mysql_result($result,0,"LeaveTime_full"); 	//업무종료 시간 전체
			...
			$ReportingBoss			= mysql_result($result,0,"ReportingBoss");			//보고 대상자-업무검토

			$EntryTime_hms = mysql_result($result,0,"EntryTime_hms"); //시분초 00:00:00  2015-01-09추가
			
			//Entry/Leave변경사항
			//기존 2에서 담는 Leave 데이터 Entry로 덮씌우기
			$code_EntryPCode2	= $code_EntryPCode;			//프로젝트코드 //김한결 2025.12.23 추가
			$EntryJobCode2		= $EntryJobCode;		//프로젝트서브코드 //김한결 2025.12.23 추가
			$ProjectNickname2	= $ProjectNickname;	//프로젝트 닉네임 //김한결 2025.12.23 추가
			$EntryJob2			= $EntryJob;			//업무내용 //김한결 2025.12.23 추가
...
// 			}
		...
		if (date("Ymd") >= $noticeDay) {
			if (empty($NewProjectCode)) {
			...
			$sql .= " From ";
			$sql .= "     project_tbl ";
			$sql .= " Where NewProjectCode = '$NewProjectCode' ";
			$sql .= " And visible_YN  = 'Y' ";
			$resource = mysql_query($sql, $db);
			
			if (!$resource) {
				echo "mysql_query(alert message resource) error";
				return;
			}
		...
		$this->assign('isActiveProjectExists', $isActiveProjectExists);
		...
		if($Status=="1" && $result_num != 0 && FN_Get_OfficeType($memberID)!="10" ){ //업무미시작 && 감리현장 근무자 제외(FN_Get_OfficeType($memberID)!="10")
			...
			//-----------------------------------------------------------------
			//-----------------------------------------------------------------
			//if($code_EntryPCode2=="" && $EntryJob2=="" && $EntryTime!=$compare_date_today){	//기존 업무 종료 미입력 인원 조건문 주석 처리 2025.12.31 김한결
			if($EntryTime!=$compare_date_today && ($LeaveTime_full == "0000-00-00 00:00:00" || $LeaveTime_full==$LeaveTime.' 18:00:18')){//업무시작일이 오늘이 아니고, 업무 종료 시간이 없으면 조건문 실행 수정 2025.12.31 김한결
...
//			echo $Status;
		...
		if($ReportingBoss!=""){
			...
			$MemSQL = "SELECT CONCAT(a.korName,CONCAT(' ',(SELECT Name FROM systemconfig_tbl WHERE SysKey='PositionCode' AND Code=a.RankCode))) AS Upper_User
						FROM member_tbl a WHERE a.MemberNo='$ReportingBoss' AND a.WorkPosition!=9";
			
			$MemRe = mysql_query($MemSQL,$db);
			
			if(mysql_num_rows($MemRe)>0){
				$ReportingBossText = mysql_result($MemRe,0,"Upper_User");
			}
		}
		//업무검토 [성명] [직급] 가져오기 종료//
		...
		$this->assign('ReportingBoss',$ReportingBoss);				//보고 대상자 사번
		$this->assign('ReportingBossText',$ReportingBossText);				//보고 대상자 성명 직급 - 업무검토
		/* ------------------------------------------------------------- */
	}//SelectWorkPlan End
	/* ******************************************************************************************* */
...
// 												}
					...
					if(($e_LeaveTime!="" || $e_LeaveTime!="0000-00-00 00:00:00") && ($e_LeavePCode!="")){
						...
						//Entry/Leave변경사항 2025.12.04
						//Leave 컬럼은 연장근무에만 사용						
						//$resultStr10	= projectToColumn($e_LeavePCode,'projectViewCode',DevConfirm($memberID));		//프로젝트코드 ViewCode : 181022 | 주석 김한결 2025.12.23 추가
						$resultStr10	= projectToColumn($resultStr04,'projectViewCode',DevConfirm($memberID));		//김한결 2025.12.23 추가
						array_push($resultArray10 ,$resultStr10);
					}else{
						//프로젝트코드 ViewCode : 181022
						$resultStr10	= projectToColumn($resultStr04,'projectViewCode',DevConfirm($memberID));		//프로젝트코드 ViewCode : 181022
					...
					if(($e_LeaveTime!="" || $e_LeaveTime!="0000-00-00 00:00:00") && ($e_LeavePCode!="")){
						...
						//Entry/Leave변경사항 2025.12.04
						//Leave 컬럼은 연장근무에만 사용
						//$set_EntryPCode = $e_LeavePCode; //주석 - 김한결 2025.12.23 추가
						$set_EntryPCode = $d_EntryPCode;//추가 - 김한결 2025.12.23 추가
					}else{
						//프로젝트코드 ViewCode : 181022
					...
					if(($e_LeaveTime!="" || $e_LeaveTime!="0000-00-00 00:00:00") && ($e_LeavePCode!="")){
						...
						//Leave 컬럼은 연장근무에만 사용
						/*업무내용_short------------------------------------*/
						//$resultStr06 = @mysql_result($re_code99,0,"e_EJ_change"); //주석 - 김한결 2025.12.23 추가
						$resultStr06 = @mysql_result($re_code99,0,"d_EJ_change");//추가 - 김한결 2025.12.23 추가
						...
						//Entry/Leave변경사항 2025.12.04
						//Leave 컬럼은 연장근무에만 사용
						//array_push($resultArray07 ,@mysql_result($re_code99,0,"e_EJ_FULL"));	//주석 - 김한결 2025.12.23 추가
						array_push($resultArray07 ,@mysql_result($re_code99,0,"d_EJ_FULL"));//추가 - 김한결 2025.12.23 추가
					}else{
						/*업무내용_short------------------------------------*/
...
*/
	...
	function BirthdayList()
	{
		...
		$birthCnt = count($birth_array);
		$this->assign('birthCnt',$birthCnt);
...
/* ******************************************************************************************* */
	...
	function LeftProcess()
	{
		...
		/* 2025.12.23 추가 - 주석처리
		$checkReportingBossMenuAuth = Array('M22014', 'M06203', 'M22006', 'B23070','M07318','B24009','M19314','B21344','B21364','B22027','B22064','B23064','T03225','B24028','B24051','B24053');
		if(array_search($memberID, $checkReportingBossMenuAuth) !== false){
			$this->assign('checkRBmenuAuth',"Y");
		}
		else{
			$this->assign('checkRBmenuAuth',"N");
		}
		*/
		...
		*/
		...
		if($memberID=="M21420" || $memberID=="M21420QQ")  //최은영과장 이정근 메뉴안보이게 처리 || $memberID=="T03225" //$memberID=="B22001"
		{
			...
			*/
			...
			//김한결 2025.12.23 추가 시작
			//업무 검토 제외 인원
			//한형관 사장님, 정태원 센터장님, 장종찬 부사장님, 양병홍 수석님, 장계석 수석님
			$reportingNot = Array('M01106','M21201','M02210','B18202','B21317');
			if(array_search($memberID, $reportingNot) !== false){
				$this->assign('reportingNot',"Y");
			}
			else{
				$this->assign('reportingNot',"N");
			}
			//김한결 2025.12.23 추가 종료
			
			if($memberID=="M22014" || $memberID=="TADMIN"){
				$this->display("intranet/common_layout/main_home.tpl");
			}
			else{
			$this->display("intranet/common_layout/main_home.tpl");
