...
function VacationBoostList_batch($Todays)
{
    ...
    if($re_num > 0) {
        ...
        while($re_row = mysql_fetch_array($re))
        {
             ...
             */
            $VacationInfo   = GetVacationInfo($db, $sel_year, $re_MemberNo ,"2017-05-30","iconv");
            //-------------------------------------------------------------------
            ...
            if($VC_vacation_gb=="0"){
                ...
                if($VacationInfo['error']==""){
                    $start_day0=$VC_entry_date;
                    $end_day0=$VC_end_date;
                    ...
                    if($diff_day < 365){
                        if($isNoticeDay3 && $VC_left_time > "0.4" && $year_end3 < $Today && $openstartday < $year_end3){
                            //회계일자 기준인원
                            //잔여연차 시간이 0.4 초과일경우
                            //연차촉진2차 고지일자(해당년도 10월31일) < 현재일자
                        ...
                        }else if($isNoticeDay2 && $VC_left_time > "0.4" && $year_end2 < $Today && $openstartday < $year_end2){
                            //회계일자 기준인원
                            //잔여연차 시간이 0.4 초과일경우
                            //연차촉진2차 고지일자(해당년도 10월31일) < 현재일자
                            $SET_vb_degree		= "2"; //고지차수
    ...
    $log_file = "../log/boost_batch_hanmac_test.txt";
    if(is_dir($log_file)){
        $log_option = 'w';
    }else{
        $log_option = 'a';
