...
class PmResisterLogic {
	...
	function ProjectKind()  //프로젝트구분
	{
		...
		$sql01= $sql01." ORDER BY SYS.orderno ASC,SYS.CodeORName ASC,SYS.Name ASC	";// 
		/* ----------------------------------- */
		$CodeName="";
		/* ----------------------------------- */
	...
	while($re_row = mysql_fetch_array($re)) {
		...
		if(strpos($re_row[p_ProjectCode], 'XX')) {
			$re_row[p_ProjectCode]="H".substr(date("Y"),2).substr($re_row[p_ProjectCode],3);
		} 
		...
		/*-----------------------------------------------*/
		//수금액
		$sql02 = "select sum(CP.CollectionPayment) cp_sum from collectionpayment_tbl CP where CP.ProjectCode = '".$re_row[p_ProjectCode]."' and (CP.CollectionDate <> '' && CP.CollectionDate <> '0000-00-00') ";
		...
		//echo $re_row[p_ProjectCode];
			...
			//2021.04.20 : 현동규 사원요청 : 해당 4개 프로젝트 코드 수금율 100% 고정
			if($re_row[p_ProjectCode]=="H16-고속-07" || $re_row[p_ProjectCode]=="H15-고속-03" || $re_row[p_ProjectCode]=="H14-고속-05" || $re_row[p_ProjectCode]=="H17-고속-04" ){
				$collectionrate='100';
			}
	...
	/* 페이지네비 관련SET End ------------------- */
		...
		$tmpGroupCode=substr($GroupCode,1,2);
		$this->smarty->assign('tmpGroupCode',$tmpGroupCode);
		$SearchDate=date("Y-m");
	...
	function PageList02()// // 관리대장 -> 페이지이동 : 외주비 지급내역서
	{
		...
		$sql02 = "select sum(DisbursePayment) DisbursePaymentSum from Outside_Pay_Disbursement_tbl where ProjectCode='".$content_id."' and DisburseDate> '1970-01-01' ";
		$result02 = mysql_query($sql02,$db);
	...
	function PageList03()// [페이지이동] 관리대장 -> 수금현황 -> 수금내역 ->수금현황 목록
	{
		...
		/*-----------------------------*/
		//수금액합계
		$sql02 = "select sum(CollectionPayment) CollectionPaymentSum from collectionpayment_tbl where ProjectCode='".$content_id."' and (CollectionDate <> '' && CollectionDate <> '0000-00-00')";
		//echo $sql02;
		$result02 = mysql_query($sql02,$db);
...
$sum_ooc_ContractAmount = 0;
		while($re_row = mysql_fetch_array($re)) {
			...
			//$re_row[ooc_TaxFree]
				...
				$re_row[ooc_Company_short] = utf8_strcut($re_row[ooc_Company],7,'..');
...
$sum_ooc_ContractAmount = $sum_ooc_ContractAmount;
	...
	if($sum!=0){
		$getPercent = ($CollectionPaymentSum_delTax / $sum)*100;
	}else{
		$getPercent = 0;
	}
...
$result = mysql_query($update_query);
	...
	//1.일반현황 Start ///////////////////////////////////////////////////////////////////////////
		...
		// 업체구성 출력시 사용됨
			...
			$p_ActualityRatio = mysql_result($result,0,"p_ActualityRatio"); 
			
			$p_Note = mysql_result($result,0,"p_Note"); 
		}else{
				//echo "프로젝트 코드(".$content_id.")가 잘못되었거나 없는 프로젝트입니다!";
				echo "프로젝트 코드(".$content_id.")";
				exit;
		}//if End
		$query_data = array(); 
		/*-----------------------------------------------*/
...
//$ActualityRatio=$re_row2[ActualityRatio];	
	...
	if($p_ActualityRatio == 0) {
		$p_ActualityRatio = 100;
	}


	//$tmppayment = $p_ContractPayment * $p_ContractRatio / 100; //용역계약금액*우리회사지분/100
	$tmppayment = $p_ContractPayment * $p_ActualityRatio / 100; //용역계약금액*우리회사지분/100
	...
	*/
	...
	if($p_ActualityRatio > 0) { 
		$val03[0] = number_format($p_ActualityRatio,2)." %"; //$val03비율[0]
	}
	/*---------------------------------*/
...
/*---------------------------------------------------------------------*/
	...
	function PageList07()	//관리대장 :  수금계획집계표
	{
		...
		$sql01=$sql01."	AND																";
		$sql01=$sql01."		P.workstatus in('수행중', '수행중지' )						";
		$sql01=$sql01."	AND																";
...
$sql03 = "select CPP.CollectionPayment cpp_sum,Note cpp_note from collectionpaymentplan_tbl CPP where CPP.ProjectCode = '".$content_id."' and CPP.PlanDateKey = '공정율'";//공정율
//echo $sql03."<br>";
$result03 = mysql_query($sql03,$db);
if(mysql_num_rows($result03) > 0){
	$cpp_sum = mysql_result($result03,0,"cpp_sum");  
	$cpp_note = mysql_result($result03,0,"cpp_note");  
}else{
	$cpp_sum ="0";
	$cpp_note="";
} 
...
$year1 = date("Y");   // 당해년도
$month1 = date("n");  // 오늘
$datekey = $year1."년".sprintf("%02d",$month1)."월";
...
$sql04 = "select sum(CPP.CollectionPayment) cpp_sum from collectionpaymentplan_tbl CPP where CPP.ProjectCode = '".$content_id."' and CPP.PlanDateKey >= '".$datekey."' and CPP.PlanDateKey <> '공정율'";
...
//echo "remainPay02".$remainPay02."<br>";
//echo "cpp_sum04".$cpp_sum04."<br>";
...
$month1 = date("n");
		for($i=1;$i<13;$i++){
			...
			$datekey = $nowYear."년".sprintf("%02d",$month1)."월";
			...
			if($re_num>0){
				...
				$ItemData=array("int_cpp_PlanDateKey" =>$datekey,"ch_cpp_CollectionPayment" =>$ch_cpp_CollectionPayment,"cpp_Note" =>$cpp_Note);
				array_push($query_data,$ItemData);

			}else{
				//$ItemData=array("int_cpp_PlanDateKey" =>$num,"ch_cpp_CollectionPayment" =>"","cpp_Note" =>"");
				$ItemData=array("int_cpp_PlanDateKey" =>$datekey,"ch_cpp_CollectionPayment" =>"","cpp_Note" =>"");
				array_push($query_data,$ItemData);
			}//if End
			$num++;
		...
		$this->smarty->assign('cpp_note',$cpp_note);//수금진행
	...
	function UpdateDB()//수정 DB실행
	{
		...
		for($i=1;$i<13;$i++){
			...
			$cpp_Date[$i]				= ($_POST['cpp_Date'.$i]==""?"22":$_POST['cpp_Date'.$i]);	// 년월일
		...
		for($j=1;$j<13;$j++){
			if( $cpp_CollectionPayment[$j] != ""){
				...
				$ch_cpp_CollectionPayment = str_replace(",","",$ch_cpp_CollectionPayment);//금액단위의 ','제거
				/*---------------------------------------*/
				//$ch_j = sprintf('%02d',$j);
		...
		$query42= $query42." ('".$content_id."','공정율','','".$cpp_sum."','".$cpp_note."', '".$date_today4."','".$MemberNo."')	";
