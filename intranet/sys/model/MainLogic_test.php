...
class MainLogic extends Smarty {
	...
	function SelectWorkPlan()
	{
		...
		$sql.= "	,(select  bb.ProjectNickname from Project_tbl bb where a.LeavePCode = replace(bb.ProjectCode, 'XX','".$shortY."')) as ProjectNickname2 	";	//프로젝트 닉네임
		$sql.= "	,a.LeaveJob                           						";	//업무내용
...
*/
	...
	function ApprovalCount()
	{
		...
		if($FormList == "") {  //일반사용자
			$sql = "select * from view_sanctiondoc_tbl where RT_SanctionState like '%".$SANCTION_CODE.":".$memberID."%' or  RT_SanctionState like '%".$SANCTION_CODE2.":".$memberID."%'";
			$re = mysql_query($sql,$db);
...
/* ******************************************************************************************* */
	...
	function VacationList(){
		...
		if($RankCode<="C8" || $memberID=="B14306test"|| $memberID=="Tadmin"){   //이사이상
			...
			$sql =	"select
					...
					where GroupCode IN ($GroupList)
						...
						) order by MemberNo, start_time";

			//echo $sql."<br>";
			$member_date = array();
			$re = mysql_query($sql,$db);	//당일 일때
			while($re_row = mysql_fetch_array($re)){
				if($member_date[$re_row[MemberNo]] == null){
					$member_date[$re_row[MemberNo]] = array();
				}
				for($i=1; $i<7; $i++){
					if(date("Y-m-d",strtotime("+".$i." day")) >= $re_row[start_time] and date("Y-m-d",strtotime("+".$i." day")) <= $re_row[end_time]){
						if(strpos($re_row[vaction_type], '반차') !== false) {		//반차일경우
							$member_date[$re_row[MemberNo]][date("Y-m-d",strtotime("+".$i." day"))] = $re_row;
						}else{	//연차일경우
							$member_date[$re_row[MemberNo]][date("Y-m-d",strtotime("+".$i." day"))] = $re_row;
						}
					}
			...
			foreach ($member_date as $key => $value) {
				//print_r($member_date[$key]);
				$s_date = '';
				for($i=1; $i<7; $i++){
					if($member_date[$key][date("Y-m-d",strtotime("+".$i." day"))] != null){
						if($s_date == ''){
							$s_date = date("Y-m-d",strtotime("+".$i." day"));
						}
					}elseif($s_date != ''){
						array_push($WeekMemberList,array( "MemberNo" => $member_date[$key][date("Y-m-d",strtotime("+".($i-1)." day"))][MemberNo], "korName" => $member_date[$key][date("Y-m-d",strtotime("+".($i-1)." day"))][korName], "vaction_type" => $member_date[$key][date("Y-m-d",strtotime("+".($i-1)." day"))][vaction_type], "start_time" => $s_date, "end_time" => $member_date[$key][date("Y-m-d",strtotime("+".($i-1)." day"))][end_time]));
						$s_date = '';
					}
				}
				if($s_date != ''){
					array_push($WeekMemberList,array( "MemberNo" => $member_date[$key][date("Y-m-d",strtotime("+6 day"))][MemberNo], "korName" => $member_date[$key][date("Y-m-d",strtotime("+6 day"))][korName], "vaction_type" => $member_date[$key][date("Y-m-d",strtotime("+6 day"))][vaction_type], "start_time" => $s_date, "end_time" => $member_date[$key][date("Y-m-d",strtotime("+6 day"))][end_time]));
				}
			}
			//print_r($WeekMemberList);
	...
	function MainHomeProcess() //메인화면 TPL
	{
		...
		*/
		...
		{	$this->display("intranet/common_layout/main_home_hidden.tpl");
			...
			if($memberID=="B14306")  //최은영과장 이정근 메뉴안보이게 처리 || $memberID=="T03225"
			{
