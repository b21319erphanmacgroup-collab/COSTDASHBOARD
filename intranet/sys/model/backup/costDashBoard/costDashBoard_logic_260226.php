...
class CostDashBoard {
	...
	function main() {
		...
		switch ($SubAction) {
			case "memberList":
				...
				$userWorkData = json_decode($jsonUserWorkData, true);
				...
				$endDay = "2026-02-28";
				$deptNo = "1020";
				$yyyy = substr($endDay, 0, 4);
				$yyyy = substr(preg_replace('/[^0-9]/', '', $endDay), 0, 4);
				$mm = substr(preg_replace('/[^0-9]/', '', $endDay), 4, 2);
				
				$betweenDay = Get_BetweenDateArray($startDay, $endDay);
				$usersResult = $this->getUserInfo($deptNo, $yyyy, $mm);
			...
			default:
				...
				$usersResult = $this->getUserInfo(null, $condYYYY, $condMM);
				if (!$usersResult["success"]) {
					echo "select user query error";
					return;
				}
				...
				$this->insertUserData($userWorkData, "1020");
	...
	function insertUserData($userWorkData, $deptNo) {
		global $db;
		
		foreach ($userWorkData As $userId => $days) {
			foreach ($days As $day => $works) {
				foreach ($works As $workDvsCd => $rows) {
					if ($workDvsCd == "dallyproject_tbl") {
						...
						for ($i = 0; $i < count($rows); $i++) {
							...
							$sql .= "     ,'$day' ";
							$sql .= "     ,'$deptNo' ";
							$sql .= "     ,'$row[VIEW_PROJ_CODE]' ";
							$sql .= "     ,'$row[VIEW_PROJ_CODE]' ";
							$sql .= "     ,'$userId' ";
							$sql .= "     ,'$row[VIEW_ACTIVITY_NAME]' ";
							$sql .= "     ,'51' ";
							$sql .= " ) ";
							echo $sql . "<br>";
							$resource = mysql_query($sql, $db);
							
							if ($row[$i]["OVER_TIME"] != "0") {
								$sql  = " Insert Into MH_WORK_SUMMARY ";
								$sql .= " ( ";
								$sql .= "     WORK_DAY ";
								$sql .= "     ,DEPT_NO ";
					...
					} else if ($workDvsCd == "userstate_tbl") {
						for ($i = 0; $i < count($rows); $i++) {
							$row = $rows[$i];
							...
							$sql .= "     '$day' ";
							$sql .= "     ,'$deptNo' ";
							$sql .= "     ,CoalEsce($row[ProjectCode], $row[NewProjectCode]) ";
							$sql .= "     ,'$row[VIEW_PROJ_CODE]' ";
							$sql .= "     ,'$userId' ";
							$sql .= "     ,'$row[VIEW_ACTIVITY_NAME]' ";
							$sql .= "     ,'$row[state]' ";
					...
					} else if ($workDvsCd == "dallyproject_addwork_tbl") {
						for ($i = 0; $i < count($rows); $i++) {
							$row = $rows[$i];
					...
					} else if ($workDvsCd == "official_plan_tbl") {
						for ($i = 0; $i < count($rows); $i++) {
							$row = $rows[$i];
	...
	function getWorkData($memberID, $currYM = null) {
		...
		$sql  = " Select ";
		$sql .= "     b.*, ";
		$sql .= "     b.EntryPCode VIEW_PROJ_CODE, ";
		$sql .= "     b.EntryJobCode VIEW_ACTIVITY_NAME, ";
		$sql .= "     ( ";
		$sql .= "         Select ";
		...
		$sql .= "     a.ProjectCode VIEW_PROJ_CODE,   ";
		...
		$sql  = " Select   ";
		$sql .= "     'dallyproject_addwork_tbl' DVS_CD,   ";
		$sql .= "     a.activity_code VIEW_ACTIVITY_NAME,  ";
		$sql .= "     a.project_code VIEW_PROJ_CODE,    ";
		$sql .= "     Round(((a.work_hour * 60) + a.work_min) / 60, 1) WORK_LABEL,   ";
		$sql .= "     Date_Format(a.EntryTime, '%Y-%m-%d') WORK_DAY,   ";
		$sql .= "     Date_Format(a.EntryTime, '%d') `DAY`,    ";
