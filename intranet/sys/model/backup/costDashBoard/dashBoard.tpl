<!DOCTYPE html>
<head>
	<meta charset="utf-8">
	<meta http-equiv="X-UA-Compatible" content="IE=edge" />
	<meta name="viewport" content="width=device-width, initial-scale=1.0, minimum-scale=1.0" />
	<!-- 보안 관련 meta -->
    <meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self' 'unsafe-inline' https://cdnjs.cloudflare.com; style-src 'self' 'unsafe-inline'; img-src 'self' data:; worker-src blob:;" />
    <meta http-equiv="X-Content-Type-Options" content="nosniff" />
    <meta http-equiv="X-XSS-Protection" content="1; mode=block" />
    <meta http-equiv="Referrer-Policy" content="strict-origin-when-cross-origin" />
    
	<title>DASH BOARD</title>
	
	<link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Saira:ital,wght@0,100..900;1,100..900&display=swap" rel="stylesheet">
    
	<!-- 스타일 -->
    <link rel="stylesheet" href="../../dashboard/css/style.css" />
	<link rel="stylesheet" type="text/css" href="../../css/common.css?ver=1.0" />
	<link rel="stylesheet" type="text/css" href="../../css/font-awesome/css/font-awesome.min.css" />
	
	<link href="../../css/common.css" rel="stylesheet" type="text/css" />
	<link href="../../css/table_code.css" rel="stylesheet" type="text/css" />
	<link href="../../css/index.css" rel="stylesheet" type="text/css" />
	<link href="../../css/myclass.css?v={$smarty.now}" rel="stylesheet" type="text/css" />
	<link href="../../js/myclass/slickSlider/slick.css" rel="stylesheet" type="text/css" />
	
	<script type="text/javascript" src="../../js/jquery/jquery-1.10.2.js"></script>
	
	
	{* <script src="../../dashboard/js/design.js"></script> *}
	<script src="../../dashboard/js/apexcharts.js"></script>
	
	<script type="text/javascript">
		let Controller = "{$Controller}";
		let ActionMode = "{$ActionMode}";
		let depts = {$depts|@json_encode};
		let users = {$users|@json_encode};
		let userWorkData = {$userWorkData|@json_encode};
		let summary = {$summary|@json_encode};
		let projectSummary = {$projectSummary|@json_encode};
	</script>
	{literal}
	<script type="text/javascript"></script>
	<style>
		/* 툴팁이 화면 (0,0)에서 이동하는 듯한 잔상 제거 */
		.apexcharts-tooltip {
			transition: none !important;
			top: 0;
			left: 0;
		}
		.display_none {
			display: none;
		}
	</style>
	
	<script>
		$(document).ready(function(){
			getDrawChart();
			drawDeptHtml("TDC");
			drawDeptHtml("GPD");
			addEventToElement("tab-btn");
			addEventToElement("navSearch");
			addEventToElement("team");
		});

		function showTeam() {
			$.each($(".tab-btn"), function(index, element){
				let container = $(element).attr("container_id");
				
				if ($(element).hasClass("active")) {
					$("#" + container).removeClass("display_none");
				} else {
					$("#" + container).addClass("display_none");
				}
			});
		}
		
		function addEventToElement(dvsCd) {
			if (dvsCd == "team") {
				$.each($(".has-submenu"), function(index, element){
					$(element).on("click", function(event){
						event.stopPropagation();
						$(element).toggleClass("open");

						let deptNo = $(event.target).attr("dept_no");
						if (!isNull(users[deptNo])) {
							if (users[deptNo].length == 0) {
								return true;
							}

							let html = "";
							for (let i = 0; i < users[deptNo].length; i++) {
								html += `<li class="sub-item">`;
									html += `<div class="sub-item-inner">${users[deptNo][i]["USER_NAME"]}</div>`;
									html += `<ul class="sub-menu depth-4"></ul>`;
								html += `</li>`;
							}

							if ($(event.target).siblings("ul[dept_no=" + deptNo + "]").children().length == 0) {
								$(event.target).siblings("ul[dept_no=" + deptNo + "]").html(html);
							}
						}
					});
				});
			} else if (dvsCd == "tab-btn") {
				$.each($(".tab-btn"), function(index, element){
					$(element).on("click", function(event){
						event.stopPropagation();
						$(element).toggleClass("active");

						showTeam();
					});
				});
			} else if (dvsCd == "navSearch") {
				$("#navSearch").on("keydown", function(event){
					if (event.key === "Enter") {
				        event.preventDefault();
				        searchUser($(this).val());
				    };
				});
			}
		}

		function searchUser(text) {
			let url = Controller;
			let params = {
				"ActionMode": ActionMode,
				"SubAction": "searchUser",
				"searchText": text
			}
			
			$.ajax({
				type:"POST",
				url: url,
				data: params,
				async: false,
				success : function(res) {
					const response = JSON.parse(res);

					
				},
				error : function(xhr, status, error) {
					alert("에러발생");
					return false;
				}
			});
		}

		function getMemberList() {
			let url = Controller;
			let params = {
				"data": JSON.stringify(userWorkData),
				"ActionMode": ActionMode,
				"SubAction": "memberList",
				"yyyy": "yyyy",
				"mm": "mm",
				"startDay": "startDay",
				"endDay": "endDay",
				"deptNo": "deptNo"
			}
			
			$.ajax({
				type:"POST",
				url: url,
				data: params,
				async: false,
				success : function(res) {
					$(".member-status").html(res);
				},
				error : function(xhr, status, error) {
					alert("에러발생");
					return false;
				}
			});
		}

		function drawDeptHtml(dvsCd) {
			let html = "";

			if (isNull(dvsCd)) {
				dvsCd = "TDC";
			}

			$(".nav-menu-wrap" + "#" + dvsCd).empty();
			for (let i = 0; i < depts[dvsCd].length; i++) {
				let row =  depts[dvsCd][i];
				let html = "";

				if (row.LVL == "1") {
					continue;
				}
				
				html = getDeptHtml(row);
				
				if (row.LVL == "2") {
					$(".nav-menu-wrap" + "#" + dvsCd).append(html);
				} else {
					$(".nav-menu-wrap" + "#" + dvsCd).find("ul[dept_no=" + row.UP_DEPT_NO + "]").append(html);
				}
			}
		}

		function getDeptHtml(row) {
			let html = "";
			
			if (row.LVL == "2") {
				html += `<li class="nav-item has-submenu" level="${row.LVL}">`;
					html += `<div class="nav-text">`;
						html += `<div class="nav-icon">`;
							if (!isNull(row["ICON_PATH"])) {
								html += `<img src="${row.ICON_PATH}" alt="">`;
							} else {
								html += `<img src="../../dashboard/images/aside/menu-icon-road.svg" alt="">`;
							}
							
						html += `</div>`;
						html += `<div class="nav-name">${row.DEPT_NAME}</div>`;
					html += `</div>`;

					html += `<div class="nav-arrow">`;
						html += `<img src="../../dashboard/images/aside/menu-arrow.svg" alt="">`;
					html += `</div>`;

					html += `<ul class="sub-menu depth-2" dept_no="${row.DEPT_NO}"></ul>`;
				html += `</li>`;

				return html;
			} else if (row.LVL == "3") {
				html += `<li class="sub-item has-submenu " level="${row.LVL}">`;
					html += `<div class="sub-item-inner" dept_no="${row.DEPT_NO}">`;
						html += `${row.DEPT_NAME}`;
						html += `<div class="nav-arrow">`;
							html += `<img src="../../dashboard/images/aside/menu-arrow.svg" alt="">`;
						html += `</div>`;	
					html += `</div>`;
					html += `<ul class="sub-menu depth-3" dept_no="${row.DEPT_NO}"></ul>`;	
				html += `</li>`;

				return html;
			} else if (row.LVL == "4") {
				html += `<li class="sub-item" level="${row.LVL}" dept_no="${row.DEPT_NO}">`;
					html += `<div class="sub-item-inner" dept_no="${row.DEPT_NO}">${row.DEPT_NAME}</div>`;
					html += `<ul class="sub-menu depth-4" dept_no="${row.DEPT_NO}"></ul>`;
				html += `</li>`;

				return html;
			} else if (row.LVL == "5") {
				html += `<li class="sub-item" level="${row.LVL}" dept_no="${row.DEPT_NO}">`;
					html += `<div class="sub-item-inner">${row.DEPT_NAME}</div>`;
				html += `</li>`;

				return html;
			}
		}

		function getUserHtml() {
			
		}

		function isNull(value) {
			if (value == null || value == undefined || value.toString().replace(/\s/g,"") == "") { 
				return true;
			}

			return false;
		}

		function getDrawChart() {
			 /* ======================================
            1. 기본 데이터 정의
            - seriesValues  : 차트 계산용 (숫자 필수)
            - displayValues : 중앙 텍스트 표시용
            ====================================== */
            const labels = ['매출', '비매출', '공통'];

            /* 차트 비율 계산용 값 (퍼센트 기준) */
            const seriesValues = [44, 33, 23];

            /* 중앙 텍스트에 보여줄 값 */
            const displayValues = [{
                text: '10시간',
                percent: '44%'
            }, {
                text: '20시간',
                percent: '33%'
            }, {
                text: '40시간',
                percent: '23%'
            }];

            /* 컬러 매칭 */
            const colors = ['#3A5A53', '#B88E5D', '#888888'];

            /* 총 시간 고정 값 */
            const TOTAL_LABEL = '총 시간';
            const TOTAL_VALUE = '5,670';

            /* 중앙 텍스트 DOM */
            const centerLabel = document.querySelector('.mh-chart-center .label');
            const centerValue = document.querySelector('.mh-chart-center .value');

            /* ======================================
            2. 중앙 텍스트 초기화 함수
            - 차트에서 마우스가 벗어났을 때 항상 복원
            ====================================== */
            function resetCenterText() {
                centerLabel.textContent = TOTAL_LABEL;
                centerValue.textContent = TOTAL_VALUE;
                centerValue.style.color = ''; // 기본 컬러로 복원
            }

            /* ======================================
            3. ApexCharts 옵션
            ====================================== */
            var options = {
                /* 차트에 실제로 사용되는 값 */
                series: seriesValues,

                chart: {
                    type: 'donut',
                    height: 200,
                    fontFamily: '"Saira", "Pretendard", sans-serif',
                    animations: {
                        enabled: false
                    },

                    /*  마우스 인터랙션 제어 */
                    events: {
                        /* 조각 호버 시 */
                        dataPointMouseEnter: function(event, chartContext, config) {
                            const idx = config.dataPointIndex;

                            centerLabel.textContent = labels[idx];
                            centerValue.innerHTML = `${displayValues[idx]
                                    .text} <span class="percent">(${displayValues[idx]
                                    .percent})</span>`;
                            centerValue.style.color = colors[idx];
                        },

                        /* 조각에서 벗어났을 때 */
                        dataPointMouseLeave: function() {
                            resetCenterText();
                        },

                        /* 차트 영역 전체를 벗어났을 때 (안전 복원) */
                        mouseLeave: function() {
                            resetCenterText();
                        }
                    }
                },

                /* ======================================
                4. 반원 도넛 설정
                ====================================== */
                plotOptions: {
                    pie: {
                        startAngle: -90,
                        endAngle: 90,
                        offsetY: -8,

                        /* ▶ 클릭 시 도넛 커지는 현상 제거 */
                        expandOnClick: false,

                        donut: {
                            size: '62%',
                            labels: {
                                show: false // 중앙 텍스트는 HTML에서 직접 제어
                            }
                        }
                    }
                },

                /* ======================================
                5. 데이터 라벨 (조각 내부 텍스트)
                ====================================== */
                dataLabels: {
                    enabled: true,
                    formatter: function(val, opts) {
                        return labels[opts.seriesIndex];
                    },
                    style: {
                        fontSize: '12px',
                        fontWeight: 600,
                        colors: ['#fff']
                    }
                },

                labels: labels,
                colors: colors,

                /* ======================================
                6. 툴팁 완전 제거
                ====================================== */
                tooltip: {
                    enabled: false
                },

                legend: {
                    show: false
                },

                /* ======================================
                7. 상태 효과 제어
                ====================================== */
                states: {
                    hover: {
                        filter: {
                            type: 'darken',
                            value: 0.9
                        }
                    },
                    active: {
                        filter: {
                            type: 'darken',
                            value: 0.9
                        }
                    }
                }
            };

            /* ======================================
            8. 차트 생성 및 렌더링
            ====================================== */
            const mhChart = new ApexCharts(document.querySelector('#mhChart'), options);

            mhChart.render();

            /* ======================================
            9. 최초 진입 시 중앙 텍스트 세팅
            ====================================== */
            resetCenterText();
		}


        <!-- 근태현황, 인트라넷 업무 검토 모달 스크립트 -->
		function modalFunc() {
            const modals = document.querySelectorAll('.confirm-modal');f
            if (!modals.length) return;

            modals.forEach((modal) => {
                modal.addEventListener('click', (e) => {
                const isCloseBtn = e.target.closest('.confirm-modal-close');
                const isConfirmBtn = e.target.closest('.confirm-modal-footer .btn-primary');
                const isOverlay = e.target.classList.contains('confirm-modal-overlay');

                if (!isCloseBtn && !isConfirmBtn && !isOverlay) return;

                e.preventDefault();

                modal.classList.remove('is-open');
                document.body.style.overflow = '';
                });
            });
		}

		 /* ======================================
        로그아웃 버튼 클릭 → 로그인 페이지 이동(퍼블리싱 단계용)
        ====================================== */
        function logout() {
	        document.addEventListener('DOMContentLoaded', function() {
	            const logoutBtn = document.querySelector('.footer-container .right-wrap');
	
	            if (!logoutBtn)
	                return;
	
	            logoutBtn.addEventListener('click', function() {
	                window.location.href = 'login.html';
	            });
	        });
		 }

		<!-- 툴팁 시작 -->
		function tooltip() {
			const trigger = document.getElementById('tooltipTrigger');
			const box = document.getElementById('tooltipBox');
			const closeBtn = document.getElementById('tooltipClose');
			
			// 1. 아이콘 클릭 시 토글
			trigger.addEventListener('click', function(e) {
			    e.stopPropagation(); // 이벤트 버블링 방지
			    const isVisible = box.style.display === 'block';
			    box.style.display = isVisible ? 'none' : 'block';
			});
			
			// 2. 닫기 버튼 클릭 시 닫기
			closeBtn.addEventListener('click', function() {
			    box.style.display = 'none';
			});
			
			// 3. 툴팁 외부 클릭 시 닫기 (사용자 편의성)
			document.addEventListener('click', function(e) {
			    if (!box.contains(e.target) && e.target !== trigger) {
			        box.style.display = 'none';
			    }
			});
		}
		<!-- 툴팁 끝 -->


	    <!-- 이력관리 시작 -->
		function etcFunc() {
	        const icon = document.getElementById('toolTipHistory');
	        const modal = document.getElementById('historyModal');
	        const closeBtn = modal.querySelector('.history-modal__close');
	
	        let isOpen = false;
	
	        function openModal() {
	            modal.style.display = 'block';
	            isOpen = true;
	        }
	
	        function closeModal() {
	            modal.style.display = 'none';
	            isOpen = false;
	        }
	
	        function toggleModal() {
	            isOpen ? closeModal() : openModal();
	        }
	
	        // hover 시 열기 (이미 열려 있으면 무시)
	        icon.addEventListener('mouseenter', () => {
	            if (!isOpen) openModal();
	        });
	
	        // 클릭 시 토글
	        icon.addEventListener('click', (e) => {
	            e.stopPropagation();
	            toggleModal();
	        });
	
	        // 모달 내부 클릭 시 닫히지 않도록
	        modal.addEventListener('click', (e) => {
	            e.stopPropagation();
	        });
	
	        // 닫기 버튼
	        closeBtn.addEventListener('click', closeModal);
	
	        // 외부 클릭 시 닫기
	        document.addEventListener('click', closeModal);
		}
	    <!-- 이력관리 끝 -->
	</script>
	{/literal}	
</head>
	<body class="page-main">
	    <!-- aside 시작 -->
		<aside class="sidebar-container">
			<div class="sidebar">
	            <!-- 로고 영역 -->
	            <div class="brand">
	                <img src="../../dashboard/images/common/logo.svg" alt="맨아워 로고">
	                <!--<img src="../../dashboard/images/aside/icon-toggle.svg" alt="사이드바 토글">-->
	            </div>
	            <!-- //로고 영역 -->
	
	            <div class="nav-wrap">
	                <!-- 메뉴 영역 -->
	                <div class="nav-container">
						<ul class="nav-menu">
							<li class="nav-item link">
	                            <div class="nav-text">
	                                <div class="nav-icon"><img src="../../dashboard/images/aside/menu-icon-integration.svg" alt=""></div>
	                                <div class="nav-name">전체 프로젝트 통합</div>
	                            </div>
	                            <div class="nav-arrow"><img src="../../dashboard/images/aside/menu-arrow.svg" alt=""></div>
	                        </li>
	
	                        <li class="nav-item link active">
	                            <div class="nav-text">
	                                <div class="nav-icon"><img src="../../dashboard/images/aside/menu-icon-person.svg" alt=""></div>
	                                <div class="nav-name">팀/개인별 분석</div>
	                            </div>
	                            <div class="nav-arrow"><img src="../../dashboard/images/aside/menu-arrow.svg" alt=""></div>
	                        </li>
	
	                        <!-- 탭 버튼 영역 시작 -->
	                        <li class="tab-button-wrap" role="tablist">
	                            <div class="box"><img src="../../dashboard/images/aside/logo-br.svg" alt="바론 로고"></div>
	                            <div class="btn tab-btn calc" container_id="GPD">GPD</div>
	                            <div class="btn tab-btn active calc" container_id="TDC">TDC</div>
	                        </li>
	                        <!-- 탭 버튼 영역 끝 -->
	
	                        <!-- 검색창 시작 -->
	                        <li class="nav-search">
	                            <form class="nav-search-form" role="search">
	                                <div class="nav-search-box">
	                                    <span class="nav-search-icon" aria-hidden="true">
	                                        <img src="../../dashboard/images/aside/icon-search.svg" alt="">
	                                    </span>
	                                    <input type="search" id="navSearch" class="nav-search-input" placeholder="팀명 또는 이름을 검색해주세요." autocomplete="off" />
	                                </div>
	                            </form>
	                        </li>
	                        <!-- 검색창 끝 -->
	                        
	                        <!--  부서 TDC -->
	                        <ul class="nav-menu-wrap display_none" id="TDC"></ul>
	                        <!--  부서 GPD -->
	                        <ul class="nav-menu-wrap display_none" id="GPD"></ul>
						</ul>
					</div>
					<!-- //메뉴 영역 -->
	
					<!-- 하단 사용자 정보 -->
					<div class="footer-container">
					    <div class="left-wrap">
					        <div class="user-info">
					            <span class="user-name">김원기 수석</span>
					            <span class="user-role">관리자</span>
					        </div>
					        <div class="company-name">기술기획팀</div>
					    </div>
					    <div class="right-wrap">
					        <img src="../../dashboard/images/aside/icon-logout.svg" alt="" class="logout">
					    </div>
					</div>
					<!-- //하단 사용자 정보 -->
            	</div>
			</div>
		</aside>
	    <!-- aside 끝 -->
	
	    <!-- 메인 컨텐츠 시작 -->
	    <main class="main-container">
	        <!-- header -->
	        <div class="page-header">
	            <div class="page-header__left">
	                <h3 class="page-header__title">상하수도팀
	                    <img src="../../dashboard/images/common/icon-information.svg" alt="이력관리" class="tool-tip-history" id="toolTipHistory">
	                </h3>
	                <!-- 이력관리 모달 시작 -->
	                <div class="history-modal" id="historyModal">
	                    <div class="history-modal__header">
	                        <span>특이사항</span>
	                        <button type="button" class="history-modal__close"><img src="../../dashboard/images/common/icon-tooltip-close.svg" alt=""></button>
	                    </div>
	                    <div class="history-modal__body">
	                        <div class="history-table">
	                            <div class="history-table__head">
	                                <span>날짜</span>
	                                <span>내용</span>
	                            </div>
	
	                            <div class="history-table__row">
	                                <span>2026.01.12</span>
	                                <span>김신지 연구원 팀 이동 (상하수도→기술기획)</span>
	                            </div>
	                            <div class="history-table__row">
	                                <span>2026.01.02</span>
	                                <span>김신지 연구원 퇴사 (2025.12.18)</span>
	                            </div>
	                            <div class="history-table__row">
	                                <span>2025.12.18</span>
	                                <span>김신지 연구원 휴직 (25.12.18~26.02.22)</span>
	                            </div>
	                            <div class="history-table__row">
	                                <span>2025.12.17</span>
	                                <span>그룹 변경 (구조→도로)</span>
	                            </div>
	                            <div class="history-table__row">
	                                <span>2025.12.11</span>
	                                <span>PM변경 (GAIA, 천지인(김신지))</span>
	                            </div>
	                        </div>
	                    </div>
	                </div>
	                <!-- 이력관리 모달 끝 -->
	                <div class="br"></div>
	                <button type="button" class="page-header__calendar-btn btn btn-gray" aria-label="2025년 12월 3주차">
	                    <img src="../../dashboard/images/common/icon-calendar.svg" alt="달력 아이콘">
	                    <span class="calendar-text">2025년 12월 3주차</span>
	                </button>
	            </div>
	
	            <div class="page-header__right">
	                <button type="button" class="page-header__setting-btn btn btn-gray" aria-label="권한 설정">
	                    <img src="../../dashboard/images/common/icon-user-setting.svg" alt="" />
	                    <span>권한 설정</span>
	                </button>
	
	                <!-- 권한 설정 모달 시작 -->
	                <div id="modalAuthority" class="modal-overlay" aria-hidden="true">
	                    <div class="modal-content">
	                        <header class="modal-header">
	                            <ul class="tab-list">
	                                <li class="active">통합 관리자</li>
	                                <li>인사 관리자</li>
	                                <li>전체 관리자</li>
	                                <li>일반 관리자</li>
	                            </ul>
	                            <button type="button" class="modal-close-btn"><img src="../../dashboard/images/common/modal-close.svg" alt=""></button>
	                        </header>
	                        
	                        <main class="modal-body">
	                        <section class="user-selection-left">
	                            <div class="search-box">
	                                <div class="search-icon">
	                                    <img src="../../dashboard/images/common/icon-search.svg" alt="검색">
	                                </div>
	                                <input type="search" placeholder="이름을 검색해주세요.">
	                            </div>
	                            <div class="dept-tree">
	                                <!-- ===== 1 DEPTH ===== -->
	                                <div class="dept-node depth-1" data-id="dev">
	                                    <p class="dept-name">기술개발센터</p>
	                                    <button class="toggle-btn">
	                                        <img src="../../dashboard/images/common/modal-toggle.svg" alt="">
	                                    </button>
	                                </div>
	
	                                <!-- ===== 2 DEPTH ===== -->
	                                <div class="dept-node depth-2" data-id="graphics" data-parent="dev">
	                                    <div class="team-wrap">
	                                        <button class="check-btn">
	                                        <img src="../../dashboard/images/common/checkbox.svg" alt="">
	                                    </button>
	                                    <p class="dept-name">그래픽스개발팀</p>
	                                    </div>
	                                    <button class="toggle-btn">
	                                        <img src="../../dashboard/images/common/modal-toggle.svg" alt="">
	                                    </button>
	                                </div>
	
	                                <!-- ===== 3 DEPTH ===== -->
	                                <div class="dept-node depth-3" data-parent="graphics">
	                                     <div class="team-wrap">
	                                        <button class="check-btn">
	                                        <img src="../../dashboard/images/common/checkbox.svg" alt="">
	                                    </button>
	                                    <p class="dept-name">Modeler셀</p>
	                                    </div>
	                                    <button class="toggle-btn">
	                                        <img src="../../dashboard/images/common/modal-toggle.svg" alt="">
	                                    </button>
	                                </div>
	
	                                <div class="dept-node depth-3" data-parent="graphics">
	                                     <div class="team-wrap">
	                                        <button class="check-btn">
	                                        <img src="../../dashboard/images/common/checkbox.svg" alt="">
	                                    </button>
	                                    <p class="dept-name">Texture셀</p>
	                                    </div>
	                                    <button class="toggle-btn">
	                                        <img src="../../dashboard/images/common/modal-toggle.svg" alt="">
	                                    </button>
	                                </div>
	                                <ul class="user-list">
	                                <li>
	                                    <button type="button" class="add-btn"><img
	                                    src="../../dashboard/images/common/icon-add.svg" alt=""></button>
	                                    <img src="../../dashboard/images/common/people-img.svg" alt="" class="thumb">
	                                        <div class="info">
	                                            <p class="company-name">
	                                                <img src="../../dashboard/images/common/logo-hanmac.svg" alt="">(주)한맥기술</p>
	                                                <p class="name">김원기
	                                                    <span>수석연구원</span>
	                                                </p>
	                                        </div>
	                                </li>
	                                <li>
	                                    <button type="button" class="add-btn">
	                                    <img src="../../dashboard/images/common/icon-add.svg" alt=""></button>
	                                    <img src="../../dashboard/images/common/people-img.svg" alt="" class="thumb">
	                                        <div class="info">
	                                            <p class="company-name">
	                                        <img src="../../dashboard/images/common/logo-hanmac.svg" alt="">(주)한맥기술</p>
	                                        <p class="name">김원기
	                                            <span>수석연구원</span>
	                                        </p>
	                                    </div>
	                                </li>
	                            </ul>
	                            </div>
	
	                            </section>
	
	                            <section class="user-selection-right">
	                                <ul class="selected-list">
	                                    <li>
	                                        <div class="selcetd-list-wrap">
	                                            <button type="button" class="remove-btn"><img src="../../dashboard/images/common/icon-delete.svg" alt=""></button>
	                                            <img src="../../dashboard/images/common/people-no-img.svg" alt="">
	                                            <div class="info">
	                                                <p class="company-name"><img src="../../dashboard/images/common/logo-hanmac.svg" alt="">(주)한맥기술</p>
	                                                <p class="name">김신지<span>연구원</span></p>
	                                            </div>
	                                        </div>
	                                        <span class="badge integrated">통합</span><!--인사:personnel ,전체:overall, 일반:general, 통합:integrated-->
	                                    </li>
	                                    <li>
	                                        <div class="selcetd-list-wrap">
	                                            <button type="button" class="remove-btn"><img src="../../dashboard/images/common/icon-delete.svg" alt=""></button>
	                                            <img src="../../dashboard/images/common/people-no-img.svg" alt="">
	                                            <div class="info">
	                                                <p class="company-name"><img src="../../dashboard/images/common/logo-br.svg" alt="">바론</p>
	                                                <p class="name">김신지<span>연구원</span></p>
	                                            </div>
	                                        </div>
	                                        <span class="badge integrated">통합</span>
	                                    </li>
	                                    <li>
	                                        <div class="selcetd-list-wrap">
	                                            <button type="button" class="remove-btn"><img src="../../dashboard/images/common/icon-delete.svg" alt=""></button>
	                                            <img src="../../dashboard/images/common/people-no-img.svg" alt="">
	                                            <div class="info">
	                                                <p class="company-name"><img src="../../dashboard/images/common/logo-halla.svg" alt="">한라산업</p>
	                                                <p class="name">김신지<span>연구원</span></p>
	                                            </div>
	                                        </div>
	                                        <span class="badge integrated">통합</span>
	                                    </li>
	                                    <li>
	                                        <div class="selcetd-list-wrap">
	                                            <button type="button" class="remove-btn"><img src="../../dashboard/images/common/icon-delete.svg" alt=""></button>
	                                            <img src="../../dashboard/images/common/people-no-img.svg" alt="">
	                                            <div class="info">
	                                                <p class="company-name"><img src="../../dashboard/images/common/logo-jh.svg" alt="">장헌</p>
	                                                <p class="name">김신지<span>연구원</span></p>
	                                            </div>
	                                        </div>
	                                        <span class="badge integrated">통합</span>
	                                    </li>
	                                    <li>
	                                        <div class="selcetd-list-wrap">
	                                            <button type="button" class="remove-btn"><img src="../../dashboard/images/common/icon-delete.svg" alt=""></button>
	                                            <img src="../../dashboard/images/common/people-no-img.svg" alt="">
	                                            <div class="info">
	                                                <p class="company-name"><img src="../../dashboard/images/common/logo-ptc.svg" alt="">피티씨</p>
	                                                <p class="name">김신지<span>연구원</span></p>
	                                            </div>
	                                        </div>
	                                        <span class="badge integrated">통합</span>
	                                    </li>
	                                    <li>
	                                        <div class="selcetd-list-wrap">
	                                            <button type="button" class="remove-btn"><img src="../../dashboard/images/common/icon-delete.svg" alt=""></button>
	                                            <img src="../../dashboard/images/common/people-no-img.svg" alt="">
	                                            <div class="info">
	                                                <p class="company-name"><img src="../../dashboard/images/common/logo-sanman.svg" alt="">삼안</p>
	                                                <p class="name">김신지<span>연구원</span></p>
	                                            </div>
	                                        </div>
	                                        <span class="badge integrated">통합</span>
	                                    </li>
	                                    <li>
	                                        <div class="selcetd-list-wrap">
	                                            <button type="button" class="remove-btn"><img src="../../dashboard/images/common/icon-delete.svg" alt=""></button>
	                                            <img src="../../dashboard/images/common/people-no-img.svg" alt="">
	                                            <div class="info">
	                                                <p class="company-name"><img src="../../dashboard/images/common/logo-hanmac.svg" alt="">(주)한맥기술</p>
	                                                <p class="name">김신지<span>연구원</span></p>
	                                            </div>
	                                        </div>
	                                        <span class="badge integrated">통합</span>
	                                    </li>
	                                </ul>
	                            </section>
	                        </main>
	
	                        <footer class="modal-footer">
	                            <button type="button" class="btn btn-white">닫기</button>
	                            <button type="button" class="btn btn-primary">저장</button>
	                        </footer>
	                    </div>
	                </div>
	                <!-- 권한 설정 모달 끝 -->
	
	                <button type="button" class="page-header__download-btn btn" aria-label="데이터 다운로드">
	                    <img src="../../dashboard/images/common/icon-download.svg" alt="" />
	                    <span>데이터 다운로드</span>
	                </button>
	            </div>
	        </div>
	
	        <!-- dashboard-content -->
	        <section class="dashboard-content">
	            <!-- 상단 요약 카드 -->
	            <div class="summary-grid" aria-label="진행 프로젝트">
	
	                <!-- ① 진행 프로젝트 -->
	                <div class="summary-card project-status">
	                    <h4>진행 프로젝트</h4>
	                    <div class="project-count">
	                        <div class="item">
	                            <span class="badge primary">주관</span>
	                            <div class="value">
	                                <strong>9</strong>
	                                <span>개</span>
	                            </div>
	                        </div>
	
	                        <div class="item">
	                            <span class="badge assist">지원</span>
	                            <div class="value">
	                                <strong>4</strong>
	                                <span>개</span>
	                            </div>
	                        </div>
	                    </div>
	                </div>
	
	                <!-- ② 프로젝트 유형별 M/H -->
	                <div class="summary-card mh-chart">
	                    <h4>프로젝트 유형별 M/H</h4>
	                    <ul class="summary-legend">
	                        <li class="legend-item is-sales">
	                            <span class="legend-dot"></span>
	                            <span class="legend-text">00시간 (00%)</span>
	                        </li>
	                        <li class="legend-item is-non-sales">
	                            <span class="legend-dot"></span>
	                            <span class="legend-text">00시간 (00%)</span>
	                        </li>
	                        <li class="legend-item is-common">
	                            <span class="legend-dot"></span>
	                            <span class="legend-text">00시간 (00%)</span>
	                        </li>
	                    </ul>
	
	                    <!-- 반원 차트 시작 -->
	                    <div class="mh-chart-mask">
	                        <div id="mhChart"></div>
	                        <div class="mh-chart-center">
	                            <span class="label">총 시간</span>
	                            <strong class="value">5,670</strong>
	                        </div>
	                    </div>
	                </div>
	
	                <!-- ③ 비용 총합 -->
	                <div class="summary-card cost-summary">
	                    <h4>비용 총합</h4>
	                    <div class="cost-list">
	                        <div class="item">
	                            <div class="bage-info">
	                                <span class="badge primary">직접비</span>
	                                <div class="tooltip-container">
	                                    <img src="../../dashboard/images/common/icon-information.svg" alt="정보" class="tool-tip-trigger" id="tooltipTrigger">
	
	                                    <div class="tooltip-box" id="tooltipBox">
	                                        <div class="tooltip-header">
	                                            <span>정보</span>
	                                            <button type="button" class="tooltip-close" id="tooltipClose"><img src="../../dashboard/images/common/icon-tooltip-close.svg" alt=""></button>
	                                        </div>
	                                        <div class="tooltip-body">
	                                            직접비 산정 근거 (표준단가) / 년도별
	                                        </div>
	                                    </div>
	                                </div>
	                            </div>
	                            <div class="value">
	                                <strong>500,000</strong>
	                                <span>천원</span>
	                            </div>
	                        </div>
	
	                        <div class="item">
	                            <span class="badge assist">간접비</span>
	                            <div class="value">
	                                <strong>500,000</strong>
	                                <span>천원</span>
	                            </div>
	                        </div>
	                    </div>
	                </div>
	                <!-- 근태 현황 -->
	                <div class="summary-card status is-leader">
		                <!-- is-normal:팀장,팀원 정상화면, is-warning: 팀원만 보이는 관리필요, is-leader: 팀장만 보이는 팀원 근태현황 -->
		                <h4>근태 현황</h4>
		
		                <!-- ① 기본 / ② 관리 필요 공용 -->
		                <div class="status-wrap">
		                    <p class="status-text">
		                        <span class="text-normal">
		                            모든 근태 현황이 <em>정상</em>입니다.
		                        </span>
		
		                        <span class="text-warning">
		                            근태 관리가 <em>필요</em>합니다.
		                        </span>
		                    </p>
		
		                    <div class="status-icon">
		                    <img src="../../dashboard/images/common/icon-good.svg" alt="">
		                    </div>
		                </div>
		
		                <!-- ③ 팀장 상세 -->
		                <div class="attendance-list">
		                    <!-- 지각 -->
		                    <div class="attendance-item late">
		                    <span class="badge">지각</span>
		                    <ul>
		                    	{foreach from=$users.$userDept item=row}
		                        <li>{$row.USER_NAME} {$GRADE_NAME}<strong>{$row.TARDY}회</strong></li>
		                        {/foreach}
		                    </ul>
		                    </div>
		
		                    <!-- 연차 -->
		                    <div class="attendance-item leave">
		                    <span class="badge">연차</span>
		                    <ul>
		                    	{foreach from=$users.$userDept item=row}
		                        <li>{$row.USER_NAME} {$GRADE_NAME}<strong>{$row.REMAIN_DAY}일 {$row.REMAIN_TIME}시간</strong></li>
		                        {/foreach}
		                    </ul>
		                    </div>
		                </div>
	                </div>
	            </div>
	
	            <!-- 프로젝트별 상세 집행 현황 -->
	            <div class="project-section">
	                <div class="title-text-wrap">
	                    <img src="../../dashboard/images/common/title-bullet.svg" alt="">
	                    <h3>프로젝트별 상세 집행 현황</h3>
	                </div>
	
	                <!-- 가로 스크롤 전용 -->
	                <div class="project-table-wrap">
	                    <div class="project-table">
	                        <!-- HEADER -->
	                        <div class="project-table-head">
	                            <div class="col name">프로젝트 정보</div>
	                            <div class="col project">투입시간/인원</div>
	                            <div class="cost-header">
	                                <div class="col mh">투입비용(천원)</div>
	                                <div class="col mh">팀 투입 비용 / 프로젝트 총 투입 비용</div>
	                            </div>
	                            <!-- <div class="col mh">투입비용(천원)</div>
	                            <div class="col mh">팀 투입 비용 / 프로젝트 총 투입 비용</div> -->
	                        </div>
	                        <div class="project-table-row-wrap">
	                        	{foreach from=$projectSummary key=projectCode item=gradeCodes}
		                        <!-- ROW 1 -->
		                        <div class="project-table-row">
		                            <!-- 프로젝트 정보 -->
		                            <div class="project-info">
		                                <div class="project-title">
		                                    <span class="dot hanmac-green"></span>
		                                    <strong>{$projectCode}</strong>
		                                </div>
		
		                                <div class="project-meta">
		                                    <div class="meta-item">
		                                        <img src="../../dashboard/images/common/icon-clipboard.svg" alt="">
		                                        <span>S/W 설계 · 조사 · 분석, 법규</span>
		                                    </div>
		                                    <div class="meta-item">
		                                        <img src="../../dashboard/images/common/icon-clock-color.svg" alt="">
		                                        <span>00.00.00</span>
		                                    </div>
		                                </div>
		                            </div>
		
									<!-- 투입시간 / 인원 (Gantt 영역) -->
			                        <div class="gantt-wrapper">
				                        <!-- ================= LEFT : 수석 / 책임 ================= -->
				                        <div class="gantt-col">
				                            <!-- 눈금 -->
				                            <div class="scale">
					                            <span>00</span><span>10</span><span>20</span><span>30</span><span>40</span>
					                            <span>50</span><span>60</span><span>70</span>
				                            </div>
				
				                            <div class="rows">
					                            <!-- 수석 -->
					                            <div class="row lead">
					                                <div class="info">수석({$gradeCodes.C0.cnt|@default:0})</div>
					                                {if $gradeCodes.C0 != ""}
					                                <div class="graph">
						                                <div class="bar blue" data-mh="22" style="width: {$gradeCodes.C0.minute/60/80*100}%;">
						                                    <span class="bar-text" title="김(2) 이(3) 박(2) 최(2) 정(2)">
						                                    김(2) 이(3) 박(2) 최(2) 정(2)
						                                    </span>
						                                </div>
						                                <span class="total blue">(Total {$gradeCodes.C0.minute/60}MH)</span>
					                                </div>
					                                {/if}
					                            </div>
					
					                            <!-- 책임 -->
					                            <div class="row manager">
					                                <div class="info">책임({$gradeCodes.E0.cnt|@default:0})</div>
													{if $gradeCodes.E0 != ""}
					                                <div class="graph">
						                                <div class="bar orange" data-mh="22" style="width: {$gradeCodes.E0.minute/60/80*100}%;">
						                                    <span class="bar-text" title="김(2) 이(3) 박(2) 최(2) 정(2)">
						                                    김(2) 이(3) 박(2) 최(2) 정(2)
						                                    </span>
						                                </div>
					                                	<span class="total orange">(Total {$gradeCodes.E0.minute/60}MH)</span>
					                                </div>
					                                {/if}
					                            </div>
				                            </div>
				                        </div>
				
				                        <!-- ================= RIGHT : 선임 / 연구원 ================= -->
				                        <div class="gantt-col">
				
				                            <!-- 눈금 -->
				                            <div class="scale">
				                            <span>00</span><span>10</span><span>20</span><span>30</span><span>40</span>
				                            <span>50</span><span>60</span><span>70</span>
				                            </div>
				
				                            <div class="rows">
					                            <!-- 선임 -->
					                            <div class="row senior">
					                                <div class="info">선임({$gradeCodes.F0.cnt|@default:0})</div>
					                            	{if $gradeCodes.F0 != ""}
					                                <div class="graph">
						                                <div class="bar purple" data-mh="22" style="width: {$gradeCodes.F0.minute/60/80*100}%;">
						                                    <span class="bar-text" title="김(2) 이(3) 박(2) 최(2) 정(2)">
						                                    김(2) 이(3) 박(2) 최(2) 정(2)
						                                    </span>
						                                </div>
						                                <span class="total purple">(Total {$gradeCodes.F0.minute/60}MH)</span>
					                                </div>
					                                {/if}
					                            </div>
					
					                            <!-- 연구원 -->
					                            <div class="row researcher">
					                                <div class="info">연구원({$gradeCodes.J0.cnt|@default:0})</div>
					                            	{if $gradeCodes.J0 != ""}
					                                <div class="graph">
						                                <div class="bar green" data-mh="22" style="width: {$gradeCodes.J0.minute/60/80*100}%;" value="{$gradeCodes.J0}">
						                                    <span class="bar-text" title="김(2) 이(3) 박(2) 최(2) 정(2)">
						                                    김(2) 이(3) 박(2) 최(2) 정(2)
						                                    </span>
						                                </div>
						                                <span class="total green">(Total {$gradeCodes.J0.minute/60}MH)</span>
					                                </div>
					                                {/if}
					                            </div>
				                            </div>
				                        </div>
									</div>
									
									
		                            <!-- 투입 비용 -->
		                            <div class="cost-box ">
		                                <div class="cost-grid">
		                                    <div class="item">
		                                        <span>인건비</span>
		                                        <div class="item-num">
		                                             <strong>100,000,000</strong>
		                                            <em>/500K</em>
		                                        </div>
		                                    </div>
		                                    <div class="item">
		                                        <span>출장비</span>
		                                        <div class="item-num">
		                                             <strong>100,000</strong>
		                                            <em>/500K</em>
		                                        </div>
		                                    </div>
		                                    <div class="item">
		                                        <span>복리후생비</span>
		                                        <div class="item-num">
		                                             <strong>100,000</strong>
		                                            <em>/500K</em>
		                                        </div>
		                                    </div>
		                                    <div class="item">
		                                        <span>기타</span>
		                                        <div class="item-num">
		                                             <strong>100,000</strong>
		                                            <em>/500K</em>
		                                        </div>
		                                    </div>
		                                </div>
		
		                                <div class="total">
		                                    <span>합계</span>
		                                    <strong>400,000</strong>
		                                </div>
		                            </div>
		                        </div>
		                        {/foreach}
	                        </div>
	                                           
	                    </div>
	                </div>
	            </div>
	
	            </div>
	            
	            <div class="member-section">
	            <!-- 인원별 현황 시작 -->
	            <section class="member-status">
					{*
	                <div class="title-text-wrap">
	                	<img src="../../dashboard/images/common/title-bullet.svg" alt="타이틀 불렛">
	                	<h3>인원별 현황</h3>
	                </div>
					
		            <!-- 가로 스크롤 전용 -->
		            <div class="member-table-wrap">
		
		                <div class="member-table">
		                    <!-- HEADER -->
		                    <div class="member-table-head">
		                        <div class="col name">이름</div>
		                        <div class="col project">투입 프로젝트</div>
		                    </div>
		
							{foreach from=$users key=deptNo item=rows}
								{if $userDept == $deptNo}
								<div class="user_list" id="team_{$deptNo}">
									{foreach from=$rows item=row}
									<div class="member-table-row">
				                        <!-- 이름 -->
				                        <div class="col name">
				                            <div class="name-wrap">
				                                <span class="people-name">{$row.USER_NAME} {$row.GRADE_NAME}</span>
												{if $row.POSITION_CODE != "&nbsp;"}
				                                <span class="badge leader">{$row.TITLE_NAME}</span>
												{/if}
				                            </div>
				                            <div class="overtime-wrap">
				                                <img src="../../dashboard/images/common/icon-overtime.svg" alt="오버타임 아이콘">
				                                <span>초과근무 1시간</span>
				                            </div>
				                            <div class="overtime-num">
				                                <span class="over-time">9/</span>
				                                <span class="default-time">8h</span>
				                            </div>
				                        </div>
				
			                        	{foreach from=$userWorkData key=user item=days}
			                        		{if $user == $row.USER_ID}
				                        		<!-- 투입 프로젝트 -->
						                        <div class="col project" user="{$user}">
				                        		{foreach from=$days key=day item=row}
				                        			<!-- 출, 퇴근 -->
					                        		{foreach from=$row.dallyproject_tbl item=dailyRow}
						                        		<div class="project-item">
							                                <div class="project-title">
							                                    <span class="dot {if $dailyRow.OVER_TIME_MINUTE != 0}red{else}blue{/if}"></span>
							                                    <strong>{$dailyRow.PROJ_NAME} / {$day}</strong>
							                                </div>
							                                <div class="calendar-wrap">
							                                    <img src="../../dashboard/images/common/icon-calendar.svg" alt="달력 아이콘">
							                                    <span class="time-date">{$dailyRow.INITALENTRY_TIME} ~ 24.02.28</span>
							                                </div>
							                                <div class="project-text-wrap">
							                                    <img src="../../dashboard/images/common/icon-clipboard.svg" alt="작업한 내용 아이콘">
							                                    <span>{$dailyRow.PROJ_DVS_CD} / {$dailyRow.EntryJobCode}</span>
							                                </div>
							                                <div class="time-wrap">
							                                    <img src="../../dashboard/images/common/icon-clock.svg" alt="투입 시간 아이콘">
							                                    <span class="time">
							                                    	{$summary.$user.$day.dallyproject_tbl.normal/60}{if $dailyRow.OVER_TIME_MINUTE != 0}<span class="add-time">+{$dailyRow.OVER_TIME_LABEL}</span>{/if}
							                                    	시간
							                                    </span>
							                                </div>
							                                <div class="line-graph">
							                                    <span class="total-time">8시간</span>
							                                    <div class="graph-wrap progress">
							                                        <div class="fill blue" style="width: 50%;"></div>
							                                        <!--blue: 기준시간, red: 초과근무,주말근무, green:출장 -->
							                                    </div>
							                                </div>
							                            </div>
													{/foreach}
													
													<!-- 추가업무 -->
													{foreach from=$row.dallyproject_addwork_tbl item=addWorkRow}
						                        		<div class="project-item">
							                                <div class="project-title">
							                                    <span class="dot blue"></span>
							                                    <strong>{$addWorkRow.PROJ_NAME} / {$day}</strong>
							                                </div>
							                                <div class="calendar-wrap">
							                                    <img src="../../dashboard/images/common/icon-calendar.svg" alt="달력 아이콘">
							                                    <span class="time-date">{$dailyRow.INITALENTRY_TIME} ~ 24.02.28</span>
							                                </div>
							                                <div class="project-text-wrap">
							                                    <img src="../../dashboard/images/common/icon-clipboard.svg" alt="작업한 내용 아이콘">
							                                    <span>{$dailyRow.PROJ_DVS_CD} / {$addWorkRow.activity_code}</span>
							                                </div>
							                                <div class="time-wrap">
							                                    <img src="../../dashboard/images/common/icon-clock.svg" alt="투입 시간 아이콘">
							                                    <span class="time">{$addWorkRow.WORK_LABEL}시간</span>
							                                </div>
							                                <div class="line-graph">
							                                    <span class="total-time">8시간</span>
							                                    <div class="graph-wrap progress">
							                                        <div class="fill blue" style="width: 50%;"></div>
							                                        <!--blue: 기준시간, red: 초과근무,주말근무, green:출장 -->
							                                    </div>
							                                </div>
							                            </div>
													{/foreach}
													
													<!-- 출장 -->
													{foreach from=$row.official_plan_tbl2 item=tripRow}
						                        		<div class="project-item">
							                                <div class="project-title">
							                                    <span class="dot green"></span>
							                                    <strong>{$tripRow.PROJ_NAME} / {$day}</strong>
							                                </div>
							                                <div class="calendar-wrap">
							                                    <img src="../../dashboard/images/common/icon-calendar.svg" alt="달력 아이콘">
							                                    <span class="time-date">{$dailyRow.INITALENTRY_TIME} ~ 24.02.28</span>
							                                </div>
							                                <div class="project-text-wrap">
							                                    <img src="../../dashboard/images/common/icon-clipboard.svg" alt="작업한 내용 아이콘">
							                                    <span>출장</span>
							                                </div>
							                                <div class="time-wrap">
							                                    <img src="../../dashboard/images/common/icon-clock.svg" alt="투입 시간 아이콘">
							                                    <span class="time">시간표기불가</span>
							                                </div>
							                                <div class="line-graph">
							                                    <span class="total-time">8시간</span>
							                                    <div class="graph-wrap progress">
							                                        <div class="fill blue" style="width: 50%;"></div>
							                                        <!--blue: 기준시간, red: 초과근무,주말근무, green:출장 -->
							                                    </div>
							                                </div>
							                            </div>
													{/foreach}
													
													<!-- 외출 -->
													{foreach from=$row.official_plan_tbl item=outRow}
						                        		<div class="project-item">
							                                <div class="project-title">
							                                    <span class="dot green"></span>
							                                    <strong>{$outRow.PROJ_NAME} / {$day}</strong>
							                                </div>
							                                <div class="calendar-wrap">
							                                    <img src="../../dashboard/images/common/icon-calendar.svg" alt="달력 아이콘">
							                                    <span class="time-date">{$outRow.INITALENTRY_TIME} ~ 24.02.28</span>
							                                </div>
							                                <div class="project-text-wrap">
							                                    <img src="../../dashboard/images/common/icon-clipboard.svg" alt="작업한 내용 아이콘">
							                                    <span>외근</span>
							                                </div>
							                                <div class="time-wrap">
							                                    <img src="../../dashboard/images/common/icon-clock.svg" alt="투입 시간 아이콘">
							                                    <span class="time">시간표기불가</span>
							                                </div>
							                                <div class="line-graph">
							                                    <span class="total-time">8시간</span>
							                                    <div class="graph-wrap progress">
							                                        <div class="fill blue" style="width: 50%;"></div>
							                                        <!--blue: 기준시간, red: 초과근무,주말근무, green:출장 -->
							                                    </div>
							                                </div>
							                            </div>
													{/foreach}
			                        			{/foreach}
						                        </div>
			                        		{/if}
										{/foreach}	
				                    </div>
				                    {/foreach}
			                    </div>
			                    {/if}
							{/foreach}
		                </div>
		            </div>
		            *}
	            </section>
	            <!-- 인원별 현황 끝 -->
	            <!-- 프로젝트별 점유율 시작 -->
	            <section class="project-ratio">
	                <div class="title-text-wrap">
	                    <img src="../../dashboard/images/common/title-bullet.svg" alt="">
	                    <h3>프로젝트별 점유율</h3>
	                </div>
	
	                <div class="ratio-sub-title">투입시간 비율</div>
	
	                <div class="ratio-content">
	                    <!-- LEFT : LIST -->
	                     <div class="ratio-list-wrap">
	                        <ul class="ratio-list">
	                        <li class="item blue">
	                            <span class="dot"></span>
	                            <strong>공학용 사이니지</strong>
	                            <span class="percent">26.9% (64h)</span>
	                            <span class="trend down">2.5% ↓</span>
	                        </li>
	
	                        <li class="item pink">
	                            <span class="dot"></span>
	                            <strong>대산~당진 시공 2공구</strong>
	                            <span class="percent">26.1% (62h)</span>
	                            <span class="trend up">3.8% ↑</span>
	                        </li>
	
	                        <li class="item green">
	                            <span class="dot"></span>
	                            <strong>GIS Mapper</strong>
	                            <span class="percent">21.8% (52h)</span>
	                            <span class="trend up">6.2% ↑</span>
	                        </li>
	
	                        <li class="item orange">
	                            <span class="dot"></span>
	                            <strong>GAIA</strong>
	                            <span class="percent">17.6% (42h)</span>
	                            <span class="trend down">10.5% ↓</span>
	                        </li>
	
	                        <li class="item gray">
	                            <span class="dot"></span>
	                            <strong>KNGIL</strong>
	                            <span class="percent">4.2% (10h)</span>
	                            <span class="trend up">3.5% ↑</span>
	                        </li>
	                    </ul>
	                     </div>
	                    <!-- RIGHT : DONUT -->
	                    <div class="donut-wrap">
	                        <img src="../../dashboard/images/common/persent-graph.svg" alt="">
	                    </div>
	                </div>
	            </section>
	            <!-- 프로젝트별 점유율 끝 -->
	            </div>
	            <!-- ===============================
	                1) 업무 검토 모달
	            =============================== -->
	            <div class="confirm-modal confirm-modal--review"><!-- is-open 클래스 추가시 모달 오픈-->
	            <div class="confirm-modal-overlay"></div>
	
	            <div class="confirm-modal-content">
	                <!-- Header -->
	                <div class="confirm-modal-header">
	                <span>확인 및 점검</span>
	                <button class="confirm-modal-close" type="button">
	                    <img src="../../dashboard/images/common/icon-modal-close.svg" alt="">
	                </button>
	                </div>
	
	                <!-- Body -->
	                <div class="confirm-modal-body">
	                <div class="confirm-modal-icon">
	                    <img src="../../dashboard/images/common/icon-warning.svg" alt="">
	                </div>
	                <p class="confirm-modal-message">
	                    <strong>인트라넷</strong>을 통해 <strong>업무 검토</strong>를 진행해 주세요.
	                </p>
	                </div>
	
	                <!-- Footer -->
	                <div class="confirm-modal-footer">
	                <button type="button" class="btn btn-primary">확인</button>
	                </div>
	            </div>
	            </div>
	
	
	            <!-- ===============================
	                2) 근태 현황 확인 모달
	            =============================== -->
	            <div class="confirm-modal confirm-modal--attendance "><!-- is-open 클래스 추가시 모달 오픈-->
	            <div class="confirm-modal-overlay"></div>
	
	            <div class="confirm-modal-content">
	                <!-- Header -->
	                <div class="confirm-modal-header">
	                <span>확인 및 점검</span>
	                <button class="confirm-modal-close" type="button">
	                    <img src="../../dashboard/images/common/icon-modal-close.svg" alt="">
	                </button>
	                </div>
	
	                <!-- Body -->
	                <div class="confirm-modal-body">
	                <div class="confirm-modal-icon">
	                    <img src="../../dashboard/images/common/icon-warning.svg" alt="">
	                </div>
	                <p class="confirm-modal-message">
	                    근태 현황의 <strong>확인</strong>이 필요합니다.
	                </p>
	                </div>
	
	                <!-- Footer -->
	                <div class="confirm-modal-footer">
	                <button type="button" class="btn btn-primary">확인</button>
	                </div>
	            </div>
	            </div>
	        </section>
	        <footer>
	            <p>Copyright(c) BARON Consultant Co.,Ltd All Rights Reserved.</p>
	        </footer>
	    </main>
	    <!-- 메인 컨텐츠 끝 -->
	     
	</body>
</html>