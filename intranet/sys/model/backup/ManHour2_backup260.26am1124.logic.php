		...
		function Manhour_my()
		{
			...
			extract($_REQUEST);
				...
				*/
				...
				{
					$item = array(
						'project_code' => $re_row3['project_code'],
						'project_name' => $re_row3['project_name'],
						'activity_code' => $re_row3['activity_code'],
		...
		function Manhour_Myproject(){
			...
			$sql2 = "
				...
				(
					SELECT
						r.ProjectCode,
						GROUP_CONCAT(mt.korName ORDER BY r.sum_minutes DESC, r.MemberNo ASC SEPARATOR ', ') AS top3_kornames
					FROM
					(
						SELECT
							s1.ProjectCode,
						...
						(
							...
							WHERE mm.WorkDate >= '" . $startDateEsc . "'
							  ...
							  AND mm.ProjectCode <> ''
							GROUP BY mm.ProjectCode, mm.MemberNo
						) s1
						LEFT JOIN
						(
							SELECT
							...
							ON s1.ProjectCode = s2.ProjectCode
						   AND (
								s2.sum_minutes > s1.sum_minutes
								OR (
									s2.sum_minutes = s1.sum_minutes
									AND s2.MemberNo < s1.MemberNo
								)
						   )
						GROUP BY s1.ProjectCode, s1.MemberNo, s1.sum_minutes
						HAVING COUNT(s2.ProjectCode) < 3
					) r
					LEFT JOIN member_tbl mt
						ON mt.memberno = r.MemberNo
					GROUP BY r.ProjectCode
				) t
			...
			if ($re2) {
				while ($row = mysql_fetch_assoc($re2)) {
					...
					$projectList[] = array(
						...
						'MyRatio'		=> round(($myMin / $rowTotalMin) * 100, 2) . '%'
					);
				}
			}
			...
			$sql3 = "
				SELECT
					...
					m.MemberNo,
					mem.korName,
					(SELECT Name FROM systemconfig_tbl WHERE SysKey = 'PositionCode' AND CODE = m.PostionCode ) AS PositionName,
					SUM(m.WorkMinutes) AS total_minutes
				FROM mh_day_alltime m
				LEFT JOIN member_tbl mem
					ON mem.MemberNo = m.memberno
				...
				ORDER BY m.ProjectCode ASC, total_minutes DESC, m.MemberNo ASC";
			$re3 = mysql_query($sql3, $db);
