...
if(DevConfirm($memberID)){
	...
	//echo 'result_num='.$result_num."<br>";
		...
		if($result_num != 0) {
			...
			$oldProjectCode2	= projectToColumn($code_EntryPCode,'oldProjectCode2');		//프로젝트코드 oldProjectCode2
...
// 			}
		...
		$this->assign('WP_oldProjectCode2',$oldProjectCode2);	//프로젝트코드 oldProjectCode2
		
		if($Status=="1" && $result_num != 0 && FN_Get_OfficeType($memberID)!="10" ){ //FN_Get_OfficeType($memberID)!="10" << 감리현장 근무자 제외
			
			if( DevConfirm($memberID)){
				//echo '='.DevConfirm($memberID).'=';
				echo 'DevConfirm='.DevConfirm($memberID);
				$this->assign('test','code_EntryPCode2=['.$code_EntryPCode2.']  : MainLogic.php : 297');
			}
		...
		}else{//FN_Get_OfficeType($memberID)=="10" << 감리현장 근무자 <== 새로운 업무종료 적용안함 : 기존 업무종료
