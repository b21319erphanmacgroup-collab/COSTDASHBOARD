<?php
	...
	* 초기화면 로딩내용 관련
	* 2015-      : 
	* 2015-02-11 : 최근업무내용(LoginInfo()) : 소스코드 고도화 : SUK 
	...
	function LoginInfo()
	{
        ...
        ************************/
		...
		for($i=0;$i<31;$i++){
			...
			$sql33 =	$sql33."	    ,DATE_FORMAT(CURDATE()-".$i.", '%d')	as a_start_timeDDCompare";	//일자 : DD
			$sql33 =	$sql33."	    ,DATE_FORMAT(CURDATE()-".$i.", '%Y-%m-%d') as a_dayCompare		";
			$sql33 =	$sql33."	    ,DAYOFWEEK(DATE_FORMAT(CURDATE()-".$i.", '%Y-%m-%d'))            as a_dayNameCompare				";	//요일(숫자:일(0)~토(6))
			...
			$sql33 =	$sql33."			WHERE														";
			$sql33 =	$sql33."				(														";
			$sql33 =	$sql33."				start_time <= DATE_FORMAT(CURDATE()-".$i.", '%Y-%m-%d')	";
			$sql33 =	$sql33."				AND														";
			$sql33 =	$sql33."				end_time >= DATE_FORMAT(CURDATE()-".$i.", '%Y-%m-%d')	";
			$sql33 =	$sql33."				)														";
			$sql33 =	$sql33."				 AND													";
...
//echo $sql33."<br><br>";
			...
			if($re_num_code33 != 0) {
				...
				/*연장근무유무 ------------------------------------*/
				$a_start_time = @mysql_result($re_code33,0,"a_start_time");
				$resultStr03  = overtimeCheck($a_start_time,$memberID,'');
				...
				array_push($resultArray05 ,"");

				/*업무내용_short------------------------------------*/
				$resultStr06 = @mysql_result($re_code33,0,"a_note");
				...
				$sql99= $sql99."	,substring(D.EntryJob,1,13)							as d_EJ_change				";	//업무내용(풀네임)
				...
				$sql99= $sql99." AND																				";
				$sql99= $sql99." DATE_FORMAT(D.EntryTime, '%Y-%m-%d')  = DATE_FORMAT(CURDATE()-".$i.", '%Y-%m-%d')	";
				/*------------------------------------*/
