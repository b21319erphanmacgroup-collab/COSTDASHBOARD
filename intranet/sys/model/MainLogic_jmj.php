...
//echo count($query_data2);
	...
	function Calendar_Mini(){
		...
		*/
		...
		if($GroupCode == '31' || $GroupCode == '33'){
			$sql2= $sql2."	S.groupcode in ('31','33')	";
		}else{
		...
		//if($memberID=="M21464"){
			...
			$sql = "
				SELECT
					...
					,(
						CASE
							WHEN STR_TO_DATE(vb_notice_dt, '%Y-%m-%d')<= STR_TO_DATE('$yyyymmdd', '%Y-%m-%d') THEN 'Y'
							ELSE 'N'
						END
					) as V_ALRAM_YN
				FROM
					vacation_boost_tbl
				where
					vb_member_no = '$memberID'
					and '$yyyymmdd' between vb_notice_dt and vb_etc_04
				order by
					vb_update_dt desc
				LIMIT 0,1
				;
			...
			if($VAC_INFO_ALRAM_YN=="Y"){
				...
				$sql = "
					SELECT
						'Y' as DOC_EXIST_YN
						, ( CASE
							WHEN RT_SanctionState LIKE '%RETURN%' THEN 'RETURN'
							WHEN RT_SanctionState LIKE '%FINISH%' THEN 'FINISH'
							ELSE 'ING'
						END ) as DOC_STATUS
					FROM
						sanctiondoc_tbl
					WHERE
						FormNum = 'HMF-3-10'
						AND Detail4 = CONCAT( '$memberID','_','$VAC_INFO_ALRAM_DATE','_','$VAC_INFO_ALRAM_DEGREE' )
					;
				";
				...
				if( $re_num == 0 ){
					$datarow_DOC[0] = array();
					$datarow_DOC[0]["DOC_EXIST_YN"] = 'N';
					$datarow_DOC[0]["DOC_STATUS"] = '';
				}else{
					while($re_row = mysql_fetch_array($re)){
						array_push( $datarow_DOC, $re_row );
					}
				}
