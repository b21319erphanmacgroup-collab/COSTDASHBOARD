		...
		if ($date1 != $sel_year) // 같은년도 아니면 모두
{
		...
		for($month = 1; $month <= $MAX_SUM; $month ++) {
			...
			if($month=="13")
			{
				$CCSUMtotal_sum = sprintf ( "%.1f", round ( $soft1_tot [$month] / $UNIT, 2 ) );
			}else
			{
				if($soft1_tot [$month]==0)
				{
					$contract_change_sum2 [$month] = 0;
				}else{
					$contract_change_sum2 [$month] = sprintf ( "%.1f", round ( $soft1_tot [$month] / $UNIT, 2 ) );
				}
			}
			/*
			echo $month."-".$contract_change_sum2 [$month]."<br>";
				...
				if ($g_code >= 140 && $g_code <= 150) // R&D
{
		...
		$this->display ( "intranet/common_contents/work_business_test/business_result_mvc.tpl" );
	...
	function ContractReportLogic() {
		...
		if ($sub_index == "1") {
			$this->display ( "intranet/common_contents/work_business_test/business_contract_mvc.tpl" );
		} else if ($sub_index == "2" or $sub_index == "3") {
			$this->display ( "intranet/common_contents/work_business_test/business_contract_mvc2.tpl" );
		}
	}
	...
	function SalesReportLogic() {
		...
		while ( $record4 = mysql_fetch_array ( $recordBlock4 ) ) {
			...
			for($i = 0; $i <= 3; $i ++) {
				...
				$azSQL = "SELECT * FROM collectionpayment_tbl WHERE ProjectCode like '%$g_name[$k]%' and DemandDate >= '$StDate' and  DemandDate < '$EdDate' order by ProjectCode asc, ContractStep asc, Kind asc";
				...
				if (mysql_num_rows ( $recordBlock5 ) > 0) {
					while ( $record5 = mysql_fetch_array ( $recordBlock5 ) ) {
						...
						$g_KindOrderCompany [$k] [$i] [$j] = Code2Name(@mysql_result ( $recordBlock6, 0, "KindOrderCompany"),"OrderCompanyCode",0);
		...
		$this->assign ( 'code_sum1', $code_sum1 );
		$this->assign ( 'tot1', $tot1 );
		$this->assign ( 'g_KindOrderCompany', $g_KindOrderCompany );
		
		
		$this->assign ( 'query_data', $query_data );
		$this->assign ( 'result_num_row', $result_num_row );
		
		$this->display ( "intranet/common_contents/work_business_test/business_sales_mvc.tpl" );
	}
	...
	function CollectionReportLogic() {
		...
		while ( $record4 = mysql_fetch_array ( $recordBlock4 ) ) {
			...
			for($i = 0; $i <= 3; $i ++) {
				...
				$azSQL = "SELECT * FROM collectionpayment_tbl WHERE ProjectCode like '%$g_name[$k]%' and CollectionDate >= '$StDate' and  CollectionDate < '$EdDate' order by ProjectCode asc, ContractStep asc, Kind asc";
		...
		$this->display ( "intranet/common_contents/work_business_test/business_collection_mvc.tpl" );
	}
	...
	function NoncontractViewLogic() {
		...
		$this->display ( "intranet/common_contents/work_business_test/business_Noncontract_mvc.tpl" );
	}
	...
	function NoncontractInputLogic() {
		...
		$this->display ( "intranet/common_contents/work_business_test/business_Noncontract_input_mvc.tpl" );
	}
		...
		} else // 추가시
{
	...
	function ConsaleGoalInputLogic() {
		...
		$this->display ( "intranet/common_contents/work_business_test/business_consalegoal_mvc.tpl" );
	}
					...
					if ($sub_index == "1") {
						...
						if (mysql_num_rows ( $recordBlock5 ) > 0) {
							while ( $record5 = mysql_fetch_array ( $recordBlock5 ) ) {
								...
								$g_OrgContractPayment[$k] [$i] [$j] = $record5 [OrgContractPayment];
								...
								*/
//2020-07-29 최동석변경요청 최초계약금액  / 1.1 
								$ContractValue [$k] [$i] [$j] = $g_OrgContractPayment [$k] [$i] [$j]/1.1;
								//echo "계약금".$ContractValue [$k] [$i] [$j]."<br>";


//2020-07-29 최동석변경요청 지분금액은 계약금액 X 실지분율
								/*
								if ($g_ActualityRatio [$k] [$i] [$j] > 0 and $g_ActualityRatio [$k] [$i] [$j] < 100) { // 당사지분율 적용
									$ActualityValue1 [$k] [$i] [$j] = ($g_ContractPayment [$k] [$i] [$j] * $g_ActualityRatio [$k] [$i] [$j] / 100);
								} else {
								...
								*/
								if ($g_ActualityRatio [$k] [$i] [$j] > 0 and $g_ActualityRatio [$k] [$i] [$j] < 100) { // 당사지분율 적용
									$ActualityValue1 [$k] [$i] [$j] = ($ContractValue [$k] [$i] [$j] * $g_ActualityRatio [$k] [$i] [$j] / 100);
								} else {
									$ActualityValue1 [$k] [$i] [$j] = $ContractValue [$k] [$i] [$j];
								}

								//echo "지분금액====".$ActualityValue1 [$k] [$i] [$j]."<br>";									
								...
								$g_KindOrderCompany [$k] [$i] [$j] = Code2Name($record5 [KindOrderCompany],"OrderCompanyCode",0);

								//해당년도에 계약금액변경이 있으면 신규에는 변경전 금액을 표시한다
//2020-07-29 최동석변경요청 지분금액 변경금액은 적용안한다
								/*
								$sql = "select * from Change_List_tbl where ProjectCode='$tmp_ProjectCode' and ChangeItem ='계약금액' and ChangeDate like '$sel_year%'";
								echo $sql."<br>"; 
								...
								*/
								...
								//echo "지분금액".$ActualityValue [$k] [$i] [$j]."<br>";
		...
		if ($sub_index == "1") {
			$this->display ( "intranet/common_contents/work_business_test/business_contract_mvc11.tpl" );
		} else if ($sub_index == "2" or $sub_index == "3") {
			$this->display ( "intranet/common_contents/work_business_test/business_contract_mvc22.tpl" );
		}
	}
