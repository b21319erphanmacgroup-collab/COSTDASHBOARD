...
//             echo "-----------------<br>";
            ...
            */
            ...
            $VacationInfo   = GetVacationInfo($db, $sel_year, $re_MemberNo,"","iconv" );
            //-------------------------------------------------------------------
            ...
            $VC_work_year		= $VacationInfo['work_year'];		//재직기간
            $VC_WORK_PERIOD_TYPE= $VacationInfo['WORK_PERIOD_TYPE']; //0=1년미만, 1=1년, 2=1년초과
            
            if($VC_WORK_PERIOD_TYPE=="0"){
                $VC_WORK_PERIOD_TYPE_KOR= "_1년미만";
            }else  if( $VC_WORK_PERIOD_TYPE=="1"){
                $VC_WORK_PERIOD_TYPE_KOR= "_1년";
            }else  if( $VC_WORK_PERIOD_TYPE=="2"){
                $VC_WORK_PERIOD_TYPE_KOR= "_1년초과";
            }
            ...
            if($VC_vacation_gb=="0"){
                $vc_vacation_gb_kor= "_회계일자기준";
            }else  if( $VC_vacation_gb=="1"){
                $vc_vacation_gb_kor= "_입사일자기준";
            }
            ...
            if($VC_vacation_gb=="0"){
                ...
                $year_end1_note="회계일_07-01".$vc_vacation_gb_kor;
                //2차_10월31일(고정)
                $year_end2=$this_year."-10-31";//2차알림촉진 고지일자
                $year_end2_note="회계일_10-31".$vc_vacation_gb_kor;
                ...
                $sql_proc_bool = false; //true false
                
                //--------------------------------------------------
                if($isNoticeDay2 && $VC_left_time > "0.4" && $year_end2 < $Today){
                    //회계일자 기준인원
                    //잔여연차 시간이 0.4 초과일경우
                    ...
                    $sql_proc_bool = true; //true false
                    
                }
                if($isNoticeDay1 && $VC_left_time > "0.4" && $year_end1 < $Today){
                    //회계일자 기준인원
                    //잔여연차 시간이 0.4 초과일경우
                ...
                if($sql_proc_bool){
                    ...
                    $sql  = "
						Delete From vacation_boost_tbl_test
						Where
							vb_member_no = '$re_MemberNo'
							And
							vb_degree = '$SET_vb_degree'
                   ...
                   // $resource = mysql_query($sql, $db);
                    ...
                    $sql="
					insert into vacation_boost_tbl_test
					(
					vb_member_no,
					vb_notice_dt,
					vb_degree,
...
//                  );
               ...
               // $year_end1_note="재직기간_1년미만_3개월전";;
                $year_end1_note="재직기간_".$VC_work_year.$VC_WORK_PERIOD_TYPE_KOR.$vc_vacation_gb_kor;
                
                //2차_만료일 1개월전
                $year_end2 = date("Y-m-d", strtotime("-1 months", strtotime($VC_end_date)));
                $year_end2_note="1년미만_1개월전";
                $year_end2_note="재직기간_".$VC_work_year.$VC_WORK_PERIOD_TYPE_KOR.$vc_vacation_gb_kor;
                
                
                //3차_만료일로부터 10일전
                $year_end3 = date("Y-m-d", strtotime("-1 day", strtotime($VC_end_date)));
                $year_end3_note="1년미만_10일전";
                $year_end3_note="재직기간_".$VC_work_year.$VC_WORK_PERIOD_TYPE_KOR.$vc_vacation_gb_kor;
                ...
                //--------------------------------------------------
                if($isNoticeDay3 && $VC_left_time > "0.4" && $year_end3 < $Today){
                    //회계일자 기준인원
                    //잔여연차 시간이 0.4 초과일경우
                    //연차촉진2차 고지일자(해당년도 10월31일) < 현재일자
        ...
        echo $sql_text_0."<br><br>";
        echo $sql_text_1."<br><br>";
    ...
    //==============================================
    //로그남기기
    $log_file = "../log/boost_batch_ptc_test.txt";
    if(is_dir($log_file)){
        $log_option = 'w';
    }else{
        $log_option = 'a';
    ...
    //$this->display("person_mng/sub/vacation_boost_tbl_test_list.tpl");
}
//월별연차사용촉진 대상자end-----
