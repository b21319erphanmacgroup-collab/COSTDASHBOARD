			...
			*/
			...
			else if($tab_index=="2") //결재할문서
			{
				...
				else  //처리부서
				{
					...
					$sql = "select * from SanctionDoc_tbl where (RT_SanctionState like '%".$SANCTION_CODE.":".$memberID."%' or RT_SanctionState like '%".$SANCTION_CODE2.":".$memberID."%') or (PG_Code='".$MyGroupCode."' and RT_SanctionState like '%".$PROCESS_RECEIVE."%' and FormNum in ($FormList)) order by RG_Date desc";
					$sql2 =$sql." limit $Start, $page";
				}
			...
			if($tab_index<>"1")
			{
				...
				while($re_row2 = mysql_fetch_array($re2))
				{
						...
						//연장근무신청서 [개인]
							if($FormNum=="PTF-9-2-s" && $tab_index == "2")
							{
								$CommandDo = "acceptdoc_all"."('".$re_row2[DocTitle]."','PTF-9-2','".$memberID."');";
								$BtnText="통합결재";
							}
							else if($FormNum=="PTF-4-5-s" && $tab_index == "2") //휴일근무
							{
								$CommandDo = "acceptdoc_all"."('".$re_row2[DocTitle]."','PTF-4-5','".$memberID."');";
								$BtnText="통합결재";
							}
							else
							{
