	...
	class VacationBoard extends Smarty {
        ...
        function VacationBoardList()
        {
            ...
            }else
            {
                ...
                {
                    $presql="select * from member_tbl where (WorkPosition = 1 or WorkPosition = 4) and RankCode NOT LIKE 'C%' and GroupCode='$GroupCode' order by IF(RealRankCode > 'C7',RankCode,IF(RealRankCode='',RankCode, IFNULL(RealRankCode,RankCode))),binary(korName)";
                }else
                {
            ...
            while($re_row = mysql_fetch_array($re))
            {
				...
				//	../../../person_mng/inc/vacationfunction.php
				$VacationInfo = GetVacationInfo($db, $ThisYear, $MemberNo );

				$re_row[cal_type] = $VacationInfo['vacation_gb'];

				$re_row[rest_day2] = $VacationInfo['rest_day_text']; //전년이월
				$re_row[rest_day] = $VacationInfo['rest_time']; //전년이월cnt
				$re_row[new_day2] = $VacationInfo['new_day_text'];  //생성연차
				$re_row[new_day] = $VacationInfo['new_time'];  //생성연차cnt
				$re_row[new_day_sum2] = $VacationInfo['total_day_text'];  //총연차
				$re_row[new_day_sum] = $VacationInfo['total_time'];  //총연차cnt
				$re_row[use_day2] = $VacationInfo['use_day_text'];  //사용연차
				$re_row[use_day] = $VacationInfo['use_time'];  //사용연차cnt
				$re_row[remaind_day2] = $VacationInfo['left_day_text'];  //잔여연차
				$re_row[remaind_day] = $VacationInfo['left_time'];  //잔여연차cnt
            ...
            // 전년이월
            //============================================================================
            $query_lastyear = "select * from diligence_tbl where date like '%$ThisYear%' and rest_day != 0";
            ...
            $this->display("intranet/common_contents/work_vacation/annual_vacation_mvc.tpl");
        }


        function VacationBoardList_back()
		{
			global $db,$memberID;
			global $sel_year,$GroupCode;
				...
				//============================================================================
					...
					}else
					{
								...
								*/
								...
								$re_row[remaind_day]=$tmpData[10];  //잔여연차cnt
...
//echo "use_hour".$use_hour."<br>";
		...
		function VacationDetailList()
		{
			...
			$VacationUsed = GetVacationUsed( $db, 'standard', $SearchID, $EntryDate, '' );

			$query_data = $VacationUsed[GetstandardYear( date("Y"), $EntryDate )];
		...
		function VacationDetailList_230314()
		{
			global $db,$memberID;
			global $sel_year,$SearchID,$vac_type,$EntryDate;
