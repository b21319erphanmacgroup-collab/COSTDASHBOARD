	...
	class ApprovalMemberLogic {
		...
		{	
			...
			}else
			{
				...
				// }
				// M23047 본사+현장근무자 전체 리스트 추가 조규형이사 요청 신광수, 이창용, 유정옥, 정재익, 조규형, 정미희, 류시우 상단으로 조정 250102 서승완
				if($gcode == "31"){		
					$subSql = " UNION select * , '1' as orderType from member_tbl where MemberNo IN ('B15202','M04602','M05305','M20402','M21423','M24072', 'M05508','M22007', 'M24057' ) and WorkPosition ='1' " ;				
				}			
				...
				(
					select *  from
					(
						select * , '2' as orderType from member_tbl where WorkPosition ='1' and GroupCode='$gcode' and memberNo not in ('B15202','M04602','M05305','M20402','M21423','M24072','M24057') 
						$subSql 
					)a left JOIN
					(
						select * from systemconfig_tbl where SysKey='PositionCode'
				...
				(
					select * from systemconfig_tbl where SysKey='GroupCode'
				)bb on aa.GroupCode=bb.Code order by orderType , RankCode ASC";
			}
