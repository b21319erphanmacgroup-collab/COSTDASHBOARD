	...
	class InsaSynchronization {
		...
		function SynchProcess(){
			...
			for( $index = 0; $index < count($datalist); $index++ ){
				...
				$azsql = "
					SELECT
						...
						, CASE
							...
							WHEN B.PAY_DIV = '10' AND B.WORKING_COMPANY <> '30' THEN '2'
							WHEN B.PAY_DIV = '20' AND B.WORKING_COMPANY <> '30' THEN '2'
							WHEN A.SERVICE_DIV = '2' THEN '5'
							ELSE '1'
						END AS WORKPOSITION
				...
				//COMMENT ON COLUMN HR_ORDE_MASTER.PAY_DIV IS '소속회사[10: 삼안, 2:한맥, 40:장헌, 50:할라, 60:피티씨, 70:현타]';
				//if( $onedata[0]['PAY_DIV'] == '20' or $onedata[0]['PAY_DIV'] == '30' or $onedata[0]['PAY_DIV'] == '99' ){
				if( $onedata[0]['PAY_DIV'] == '10' or $onedata[0]['PAY_DIV'] == '20' or $onedata[0]['PAY_DIV'] == '30' or $onedata[0]['PAY_DIV'] == '99' ){
					$azsql .="
						, RankCode
						/* , RealRankCode */
						, groupcode
				...
				if( $onedata[0]['PAY_DIV'] == '10' or $onedata[0]['PAY_DIV'] == '20' or $onedata[0]['PAY_DIV'] == '30' or $onedata[0]['PAY_DIV'] == '99' ){
					...
					";
				}elseif( $MODE_TAG_CHECK > 0 ){		//사간전입시 기본 초기값 셋팅
					$azsql .="
						/* RankCode		*/ , ( select Code from systemconfig_tbl where SysKey = 'PositionCode' and Name = '".$this->HangleEncode($onedata[0]['GRADE_NAME'])."' order by Code limit 1 )
						/* RealRankCode	 , ( select Code from systemconfig_tbl where SysKey = 'PositionCode' and Name = '".$this->HangleEncode($onedata[0]['AAAAA'])."' order by Code limit 1 ) */
						/* groupcode	*/ , '".$onedata[0]['GROUPCODE']."'
				...
				//COMMENT ON COLUMN HR_ORDE_MASTER.PAY_DIV IS '소속회사[10: 삼안, 2:한맥, 40:장헌, 50:할라, 60:피티씨, 70:현타]';
				//if( $onedata[0]['PAY_DIV'] == '20' or $onedata[0]['PAY_DIV'] == '30' or $onedata[0]['PAY_DIV'] == '99' ){
				//왜? 삼안은 RankCode와 groupcode를 제외했는가? $onedata[0]['PAY_DIV'] == '10' (삼안)
				if( $onedata[0]['PAY_DIV'] == '10' or $onedata[0]['PAY_DIV'] == '20' or $onedata[0]['PAY_DIV'] == '30' or $onedata[0]['PAY_DIV'] == '99' ){
					$azsql .="
						, RankCode		= ( select Code from systemconfig_tbl where SysKey = 'PositionCode' and Name = '".$this->HangleEncode($onedata[0]['GRADE_NAME'])."' order by Code limit 1 )
						/* , RealRankCode	= ( select Code from systemconfig_tbl where SysKey = 'PositionCode' and Name = '".$this->HangleEncode($onedata[0]['AAAAA'])."' order by Code limit 1 ) */
				...
				//근무회사 정보수정
				if( ( $onedata[0]['PAY_DIV'] == '20' or $onedata[0]['PAY_DIV'] == '30' ) and $onedata[0]['WORKING_COMPANY'] != '30' ){
			...
			if( $contents != 'N' ){
				...
				if($action_sql){
					...
					/*한맥 DB정보*/
					$db_hostname_HANMAC = 'erp.hanmaceng.co.kr';
					$db_database_HANMAC = 'hanmacerp';
					$db_username_HANMAC = 'root';
					$db_password_HANMAC = 'erp';

					/*한맥 DB연결----------------------------------------------------------------------*/
					$db_hanmac	= mysql_connect($db_hostname_HANMAC,$db_username_HANMAC,$db_password_HANMAC);
						if(!$db_hanmac) die ("Unable to connect to MySql : ".mysql_error());
					/*-----------------------------------------------------------------------*/

					mysql_select_db($db_database_HANMAC);
					mysql_set_charset("utf-8",$db_hanmac);
				...
				for( $index = 0; $index < count($datalist); $index++ ){
					...
					$azsql_memberno = "
						select MemberNo from member_tbl
						where
							juminno = '".$onedata[0]['JUMINNO']."'
						order by EntryDate desc, MemberNo desc
						limit 1;
					 ";
					...
					if( $onedata[0]['WORKING_COMPANY'] == '10' or $onedata[0]['WORKING_COMPANY'] == '50' ){	//삼안, 한라는 직위명칭도 있음.
						...
						$azsql .= "
							, ViewRankName		= (
								select Name from systemconfig_tbl where SysKey = 'PositionCode' and Code = '".$onedata[0]['WORKING_RANK_CODE']."'
							)
						";
					}

					$azsql .= " where MemberNo = ";

					if($action_sql){
						$this->insert_insa_log($azsql);
						if( $onedata[0]['WORKING_COMPANY'] == '10' ){	//삼안
							$re = mysql_query($azsql_memberno,$db_saman);
							while($re_row = mysql_fetch_array($re)) {
								$azsql .= " '".$re_row['MemberNo']."' ; ";
							}
							mysql_query($azsql, $db_saman);
						}elseif( $onedata[0]['WORKING_COMPANY'] == '20' ){	//한맥
							$re = mysql_query($azsql_memberno,$db_hanmac);
							while($re_row = mysql_fetch_array($re)) {
								$azsql .= " '".$re_row['MemberNo']."' ; ";
							}
							mysql_query($azsql, $db);
						}elseif( $onedata[0]['WORKING_COMPANY'] == '30' ){	//바론
							$re = mysql_query($azsql_memberno,$db);
							while($re_row = mysql_fetch_array($re)) {
								$azsql .= " '".$re_row['MemberNo']."' ; ";
							}
							mysql_query($azsql, $db);
						}elseif( $onedata[0]['WORKING_COMPANY'] == '40' ){	//장헌
							$re = mysql_query($azsql_memberno,$db_jang);
							while($re_row = mysql_fetch_array($re)) {
								$azsql .= " '".$re_row['MemberNo']."' ; ";
							}
							mysql_query($azsql, $db_jang);
						}elseif( $onedata[0]['WORKING_COMPANY'] == '50' ){	//한라
							$re = mysql_query($azsql_memberno,$db_halla);
							while($re_row = mysql_fetch_array($re)) {
								$azsql .= " '".$re_row['MemberNo']."' ; ";
							}
							mysql_query($azsql, $db_halla);
						}elseif( $onedata[0]['WORKING_COMPANY'] == '60' ){	//PTC
							$re = mysql_query($azsql_memberno,$db_ptc);
							while($re_row = mysql_fetch_array($re)) {
								$azsql .= " '".$re_row['MemberNo']."' ; ";
							}
