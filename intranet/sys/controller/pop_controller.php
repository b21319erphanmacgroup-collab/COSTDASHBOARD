...
<?
	...
	/* 로직클래스----------------------------------*/
	if($get_memberID!="M22014"){
	include "../../sys/model/PopLogic.php";
	}elseif($get_memberID=="M22014"){
	//include "../../sys/model/PopLogic_khg.php";
		include "../../sys/model/PopLogic.php";
	}
	/*---------------------------------------------*/
	...
	else if($ActionMode=="OverWorkEditPop"){			//페이지이동 : 연장근무 수정 페이지
		$GoLogic->OverWorkEditPop();
	}
	else if($ActionMode=="overWorkEdit"){			//연장근무 수정 DB 실행
		$GoLogic->OverWorkEdit();
	}
	...
	}else if($ActionMode=="SecuPledgePop"){				//보안서약서 팝업
		$GoLogic->SecuPledgePop();
	}else if($ActionMode=="SubmitConsent"){				//보안서약서 동의
		$GoLogic->SubmitConsent();
	}else{
