...
<div style="float:left;width:100%;">
	...
	<tr>
		...
		<td class="gray_bg tbl_bottom_border tbl_right_border" height="30">매출액</td>
	...
	while($record4 = mysql_fetch_array($recordBlock4)) {
		...
		if($g_code <= $LAST_GROUP_CODE_NO) {  //"정통" "경영" "영업" "관리" "교휴"
			...
			for($k=1; $k<=4; $k++) {
				...
				//$azSQL = "SELECT * FROM collectionpayment_tbl WHERE ProjectCode like '%$g_name%' and DemandDate >= '$StDate' and  DemandDate < '$EdDate' order by ProjectCode asc, ContractStep asc, Kind asc";

				$azSQL = "SELECT * FROM group_div_tbl A, collectionpayment_tbl B WHERE A.ProjectCode = B.projectCode and DivCodeName = '$g_name' and DemandDate >= '$StDate' and DemandDate < '$EdDate'";
