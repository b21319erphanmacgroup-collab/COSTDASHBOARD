	...
	<div class="member-table">
		...
		<div class="user_list" id="team_{$deptNo}">
			...
			<div class="member-table-row" user="{$user.USER_ID}">
                ...
                <div class="col name">
                    <div class="name-wrap">
                        <span class="people-name">{$user.USER_NAME} </span><span class="people-position">{$user.GRADE_NAME}</span>
                    ...
                    <div class="overtime-num">
                        ...
                        <span class="default-time">({$maxWorkHour.normal}+{$maxWorkHour.holy})h</span>
                    </div>
                </div>
	            	...
	            	{foreach from=$userWorkData[$user.USER_ID] key=projCode item=row}
						...
						<div class="project-item" type="CLOCK_IN">
							...
							<div class="calendar-wrap">
								...
								<span class="time-date">{$startDay} ~ {$endDay}</span>
							</div>
							...
							<div class="line-graph">
								<span class="total-time">{$maxWorkHour.total|string_format:"%.1f"} 시간</span>
								<div class="graph-wrap progress">
									<div class="fill blue" style="width: {$row.0.MH_SUM/60/$maxWorkHour.total*100|string_format:"%.1f"}%;"></div>
									<div class="fill red" style="width: {$row.0.OVER_MH/60/$maxWorkHour.total*100|string_format:"%.1f"}%;"></div>
					...
					{foreach from=$businessTrip[$user.USER_ID] key=projCode item=row}
						...
						<div class="project-item" type="CLOCK_IN">
							...
							<div class="line-graph">
								<span class="total-time">{$maxWorkHour.total|string_format:"%.1f"} 시간</span>
								<div class="graph-wrap progress">
									<div class="fill blue" style="width: {$row.0.MH_SUM/60/$maxWorkHour.total*100}%;"></div>
									<!--blue: 기준시간, red: 초과근무,주말근무, green:출장 -->
								</div>
							</div>
						</div>
